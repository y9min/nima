...
enum TunnelLifecycleDiagnostics {
    ...
    static func isExternalKillSignature(
        ...
        guard diagnosticHoldSeconds >= NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds else { return false }
        ...
        return cadence >= NimaConstants.vpnLifecycleExternalKillCadenceMinSeconds &&
    ...
    static func externalKillReconnectGate(
        ...
        nowTS: TimeInterval,
        windowSeconds: TimeInterval = NimaConstants.vpnLifecycleExternalKillReconnectWindowSeconds,
        maxAttempts: Int = NimaConstants.vpnLifecycleExternalKillReconnectMaxAttemptsPerWindow
    ...
    static func reconnectBreakerShortUnknownDropDecision(
        ...
        finalCause: String,
        windowSeconds: TimeInterval = NimaConstants.reconnectBreakerShortUnknownDropWindowSeconds,
        threshold: Int = NimaConstants.reconnectBreakerShortUnknownDropThreshold
...
final class SOCKSProxyServer: TikTokIPHintReporting {
    ...
    private let blockSuppressionCooldown: TimeInterval = NimaConstants.blockSuppressionCooldown
    private let aggressiveBlockSuppressionCooldown: TimeInterval = NimaConstants.aggressiveBlockSuppressionCooldown
    ...
    private let maxSnapshotHistory = NimaConstants.extensionPressureMaxSnapshotHistory
    private let maxEvents = NimaConstants.extensionPressureMaxEvents
    ...
    init(filter: ConnectionFilter) {
        ...
        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        if let stored = defaults?.object(forKey: NimaConstants.transportProtectionV2StabilityFirstKey) as? Bool {
        ...
        if let container = FileManager.default.containerURL(
            ...
            self.statsFileURL = container.appendingPathComponent(NimaConstants.statsFileName)
        } else {
    ...
    func start(callbackQueue: DispatchQueue = .global(qos: .userInitiated), ready: @escaping (Error?) -> Void) {
        ...
        params.requiredLocalEndpoint = NWEndpoint.hostPort(
            host: NWEndpoint.Host(NimaConstants.socksBindAddress),
            port: anyPort
        )
        ...
        if stabilityFirstModeEnabled {
            protectionStartupGraceUntil = Date().addingTimeInterval(NimaConstants.stabilityFirstStartupGraceSeconds)
        } else {
    ...
    private func configureUDPStartupCrashGuard(now: Date = Date()) {
        ...
        udpStartupSerialUntil = now.addingTimeInterval(NimaConstants.udpStartupSerialModeSeconds)

        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        udpCrashGuardHits = defaults?.integer(forKey: NimaConstants.udpCrashGuardHitsKey) ?? 0
        udpCrashGuardReason = defaults?.string(forKey: NimaConstants.udpCrashGuardReasonKey) ?? ""
        udpCrashGuardUntil = Date(timeIntervalSince1970: defaults?.double(forKey: NimaConstants.udpCrashGuardUntilKey) ?? 0)

        let finalCause = defaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey) ??
            defaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? ""
        let lastPhase = defaults?.string(forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey) ?? ""
        let lastDecoderEvent = defaults?.string(forKey: NimaConstants.udpLastDecoderEventJSONKey) ?? ""
        if Self.shouldActivateUDPStartupCrashGuard(
            ...
            udpCrashGuardUntil = now.addingTimeInterval(NimaConstants.udpCrashGuardSerialModeSeconds)
            udpCrashGuardHits += 1
            defaults?.set(udpCrashGuardUntil.timeIntervalSince1970, forKey: NimaConstants.udpCrashGuardUntilKey)
            defaults?.set(udpCrashGuardReason, forKey: NimaConstants.udpCrashGuardReasonKey)
            defaults?.set(udpCrashGuardHits, forKey: NimaConstants.udpCrashGuardHitsKey)
        } else if udpCrashGuardUntil <= now {
            udpCrashGuardReason = ""
            defaults?.set("", forKey: NimaConstants.udpCrashGuardReasonKey)
        }
    ...
    private func startStatsTimer() {
        ...
        timer.schedule(deadline: .now() + NimaConstants.statsInterval, repeating: NimaConstants.statsInterval)
    ...
    private func startUDPSweepTimer() {
        let timer = DispatchSource.makeTimerSource(queue: queue)
        timer.schedule(
            deadline: .now() + NimaConstants.tiktokHardeningSweepInterval,
            repeating: NimaConstants.tiktokHardeningSweepInterval
        )
        timer.setEventHandler { [weak self] in
    ...
    private func recordProviderPhase(_ phase: String) {
        ...
        defaults?.set(phase, forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey)
        defaults?.set(ts, forKey: NimaConstants.vpnLifecycleProviderLastPhaseTSKey)
        appendProviderPhaseRing(defaults: defaults, phase: phase, ts: ts, source: "socks_proxy")
    }

    private func providerLastPhase() -> String {
        UserDefaults(suiteName: NimaConstants.appGroupID)?
            .string(forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey) ?? "unknown"
    }

    private func defaultsString(_ key: String, fallback: String) -> String {
        UserDefaults(suiteName: NimaConstants.appGroupID)?.string(forKey: key) ?? fallback
    }

    private func defaultsBool(_ key: String) -> Bool {
        UserDefaults(suiteName: NimaConstants.appGroupID)?.bool(forKey: key) ?? false
    }

    private func appendProviderPhaseRing(defaults: UserDefaults?, phase: String, ts: TimeInterval, source: String) {
        guard let defaults else { return }
        var ring = decodeJSONArray(defaults.string(forKey: NimaConstants.providerPhaseRingJSONKey))
        ...
        writeJSONObject(ring, key: NimaConstants.providerPhaseRingJSONKey, defaults: defaults)
    }
    ...
    private func recordLastControlStream(state: UDPStreamState, reason: String? = nil) {
        ...
        writeJSONObject(payload, key: NimaConstants.udpLastControlStreamJSONKey, defaults: defaults)
    }
    ...
    private func recordLastDNSClose(
        ...
        writeJSONObject(payload, key: NimaConstants.udpLastDNSCloseJSONKey, defaults: defaults)
    }
    ...
    private func recordLastDecoderEvent(
        ...
        writeJSONObject(payload, key: NimaConstants.udpLastDecoderEventJSONKey, defaults: defaults)
    }
    ...
    private func startSnapshotTimer() {
        ...
        timer.schedule(
            deadline: .now() + 1.0,
            repeating: NimaConstants.extensionPressureSnapshotIntervalSeconds
        )
        timer.setEventHandler { [weak self] in
            self?.writeSnapshot()
        }
    ...
    private func writeSnapshot() {
        ...
        let stats = StatsSnapshot(
            ...
            startupStabilityPhase: defaultsString(NimaConstants.vpnLifecycleStartupStabilityPhaseKey, fallback: "unknown"),
            startupProbeCompleted: defaultsBool(NimaConstants.vpnLifecycleStartupProbeCompletedKey),
            dnsStartupDrainActive: isDNSStartupDrainActive(),
            dnsStartupDrainCloses: dnsStartupDrainCloses,
            dnsStartupDrainFramesProcessed: dnsStartupDrainFramesProcessed,
            earlyReconnectSuppressed: defaultsBool(NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey),
            iosSafeModeReason: defaultsString(NimaConstants.vpnLifecycleIOSSafeModeReasonKey, fallback: ""),
    ...
    private func handleNewConnection(_ client: NWConnection) {
        ...
        if activeConnectionCount > NimaConstants.maxConnections {
            log.log("SOCKS5: Active connection limit reached (\(activeConnectionCount)/\(NimaConstants.maxConnections)), rejecting #\(connectionCount)")
            statsErrors += 1
            recordEvent(type: .error, connId: connectionCount, host: "?", port: 0, detail: "Connection limit reached (\(activeConnectionCount)/\(NimaConstants.maxConnections))")
            activeConnectionCount -= 1
            client.cancel()
            return
        }
    ...
    private static func dnsFastLaneInputFormat(data: Data, decoder: UDPControlStreamDecoder) -> String {
        ...
        if prefix > 0, prefix <= NimaConstants.maxUDPFrameSize {
    ...
    private func startTCPSNIGatedConnect(
        ...
        client.send(content: reply, completion: .contentProcessed { [weak self] error in
            ...
            self.queue.asyncAfter(deadline: .now() + NimaConstants.tcpSNIGateTimeout, execute: timeout)
    ...
    private func readTCPSNIGateBytes(
        ...
        let remaining = NimaConstants.tcpSNIGateMaxBufferedBytes - state.buffer.count
        ...
        client.receive(minimumIncompleteLength: 1, maximumLength: remaining) { [weak self] data, _, isComplete, error in
            ...
            if let data, !data.isEmpty {
                ...
                case .needsMore:
                    if state.buffer.count >= NimaConstants.tcpSNIGateMaxBufferedBytes || state.probeTimedOut {
                        self.finishTCPSNIGateAllow(
                            client: client,
                            id: id,
                            host: host,
    ...
    private func probeTLSClientHelloSNI(_ data: Data) -> TLSClientHelloProbe {
        ...
        guard recordEnd <= NimaConstants.tcpSNIGateMaxBufferedBytes else { return .noSNI }
    ...
    private func handleFwdUDP(
        ...
        let graceAdjustedLimits = Self.startupGraceAdjustedUDPLimits(
            ...
            globalMaxActive: NimaConstants.maxActiveUDPControlStreams,
            globalMaxQueued: NimaConstants.maxQueuedUDPControlStreams,
        ...
        let graceAdjustedCreateRateCapacity = Self.startupGraceAdjustedUDPCreateRateCapacity(
            ...
            globalCreateRateCapacity: NimaConstants.udpAdmissionCreateRateCapacity,
            graceActive: isStartupGraceActive(now: now),
            safeMode: safeMode
        )
        let effectiveGlobalMaxActive = safeMode
            ? NimaConstants.safeModeMaxActiveUDPControlStreams
            : NimaConstants.maxActiveUDPControlStreams
        let effectiveGlobalMaxQueued = safeMode
            ? NimaConstants.safeModeMaxQueuedUDPControlStreams
            : NimaConstants.maxQueuedUDPControlStreams
        let effectiveCreateRatePerSecond = safeMode
            ? NimaConstants.safeModeUDPAdmissionCreateRatePerSecond
    ...
    static func currentUDPForwardingMode(defaults: UserDefaults?) -> UDPForwardingMode {
        let selective = defaults?.object(forKey: NimaConstants.udpSelectiveSafeModeEnabledKey) as? Bool
        let legacyDisabled = defaults?.object(forKey: NimaConstants.udpForwardingDisabledKey) as? Bool
        let diagnosticFastReject = defaults?.object(forKey: NimaConstants.udpDisabledFastRejectEnabledKey) as? Bool
    ...
    private func recordUDPDisabledFastReject(id: Int, host: String, port: UInt16, now: Date = Date()) {
        ...
        let logWindow = NimaConstants.udpDisabledFastRejectLogWindowSeconds
        ...
        if var state = udpDisabledRejectLogStateByDestination[destination],
           now.timeIntervalSince(state.windowStartedAt) <= logWindow {
            ...
            if state.suppressedHits % NimaConstants.udpDisabledFastRejectSummaryEvery == 0 {
                log.log(
        ...
        if udpDisabledRejectLogStateByDestination.count > NimaConstants.extensionPressureMaxSuppressionEntries {
            let overflow = udpDisabledRejectLogStateByDestination.count - NimaConstants.extensionPressureMaxSuppressionEntries
    ...
    private func startDNSFastLaneControlStream(client: NWConnection, id: Int, initialBytes: Data) {
        ...
        let state = UDPStreamState(
            id: id,
            client: client,
            decoder: UDPControlStreamDecoder(maxFrameSize: NimaConstants.maxUDPFrameSize, maxResyncAttempts: 0),
    ...
    private func readDNSFastLaneFrame(client: NWConnection, state: UDPStreamState) {
        ...
        client.receive(minimumIncompleteLength: 1, maximumLength: NimaConstants.maxUDPFrameSize + 8) { [weak self] data, _, isComplete, error in
    ...
    private func consumeDNSFastLaneBytes(
        ...
        if state.decoder.currentMode == nil {
            ...
            case .needMoreBytes(let kind):
                if !isComplete, inputData.count <= NimaConstants.maxUDPFrameSize {
    ...
    private func noteDNSFastLaneParseFailureForSummary(stage: String, detail: String) {
        ...
        if dnsFastLaneParseFailureSummaryWindowStartedAt == nil {
            ...
            queue.asyncAfter(deadline: .now() + NimaConstants.dnsFastLaneParseFailureSummaryWindowSeconds, execute: work)
        }
    ...
    private func noteDNSFastLaneParseFailureForSafetyValve(stage: String, detail: String, now: Date = Date()) {
        dnsFastLaneParseFailureTimestamps = dnsFastLaneParseFailureTimestamps.filter {
            now.timeIntervalSince($0) <= NimaConstants.dnsFastLaneDisableFailureWindowSeconds
        }
        dnsFastLaneParseFailureTimestamps.append(now)

        guard !dnsFastLaneDisabledForSession,
              dnsFastLaneParseFailureTimestamps.count >= NimaConstants.dnsFastLaneDisableFailureThreshold else {
            return
        }
        ...
        log.logAndFlush(
            "DNS_FAST_LANE disabled_for_session reason=\(dnsFastLaneDisabledReason) failures=\(dnsFastLaneParseFailureTimestamps.count) window_s=\(String(format: "%.2f", NimaConstants.dnsFastLaneDisableFailureWindowSeconds)) fallback=generic_udp_relay"
        )
    }
    ...
    private func startAcceptedUDPControlStream(
        ...
        let state = UDPStreamState(
            id: id,
            client: client,
            decoder: UDPControlStreamDecoder(maxFrameSize: NimaConstants.maxUDPFrameSize),
            trafficClass: trafficClass
        )
    ...
    private func consumeUDPControlStreamBytes(client: NWConnection, state: UDPStreamState, data: Data, source: String) {
        ...
        if state.hardeningEnabled && state.pendingFrames.count > NimaConstants.tiktokHardeningMaxPendingFramesPerStream {
        ...
        case .recovered(let decodeError):
            ...
            let now = Date()
            let failuresInWindow = state.decoderFailuresInWindow(now: now, window: NimaConstants.udpDecodeFailureWindowSeconds)
        ...
        case .failed(let decodeError):
            ...
            if case .badLength = decodeError {
                let failuresInWindow = state.decoderFailuresInWindow(now: Date(), window: NimaConstants.udpDecodeFailureWindowSeconds)
            ...
            if (state.decoderFailureCount >= effectiveDecodeFailureCloseThreshold() || density >= effectiveDecoderCloseDensity()) && !hasAnyProtectionGrace() {
                ...
                udpDecodeCloseAfterFailureThreshold += 1
                decoderErrorDensityCloses += 1
                log.log("UDP_DECODER stream=\(state.id) event=close_after_failures reason=\(reasonCode) threshold=\(NimaConstants.udpDecodeFailureCloseThreshold)")
    ...
    private func processNextUDPFrame(client: NWConnection, state: UDPStreamState) {
        ...
        if decision.action == .blockNow {
            let blockedDecisionCount = state.blockedDecisionsInWindow(
                now: Date(),
                window: NimaConstants.blockedStormRetireWindowSeconds
            )
            ...
            if safeMode,
               ...
               blockedTargetCount >= NimaConstants.selectiveSafeModeBlockedDatagramsBeforeRetire {
                statsBlocked += 1
        ...
        relayUDPDatagram(
            ...
            if let responseFrame {
                ...
                let timeoutThreshold = state.hardeningEnabled
                    ? NimaConstants.tiktokHardeningTimeoutStreakCloseThreshold
                    : NimaConstants.udpGlobalTimeoutStreakCloseThreshold
    ...
    private func relayUDPDatagram(
        ...
        if let dedupKey {
            if inflightDNSRequests.count >= NimaConstants.dnsInflightMaxEntries {
        ...
        queue.asyncAfter(deadline: .now() + NimaConstants.udpRelayTimeout, execute: timeout)
                    ...
                    udp.receiveMessage { respData, _, _, recvError in
                        if let respData, !respData.isEmpty {
                            ...
                            guard let framedData = UDPControlFrameCodec.buildResponseFrame(
                                ...
                                maxFrameSize: NimaConstants.maxUDPFrameSize
                            ) else {
                                self?.statsErrors += 1
            ...
            case .ready:
                ...
                tcp.send(content: query, completion: .contentProcessed { error in
                    ...
                    tcp.receive(minimumIncompleteLength: 2, maximumLength: 2) { lengthData, _, _, lengthError in
                        ...
                        guard responseLength > 0, responseLength <= NimaConstants.maxUDPFrameSize else {
    ...
    private func continueDNSStartupDrain(client: NWConnection, state: UDPStreamState, now: Date = Date()) {
        if state.dnsStartupDrainStartedAt == nil {
            ...
            log.log(
                "UDP_DNS startup_drain_start stream=\(state.id) window_s=\(String(format: "%.2f", NimaConstants.dnsStartupDrainWindowSeconds)) idle_s=\(String(format: "%.2f", NimaConstants.dnsStartupDrainIdleSeconds))"
            )
        }
        ...
        if let startedAt = state.dnsStartupDrainStartedAt,
           now.timeIntervalSince(startedAt) >= NimaConstants.dnsStartupDrainMaxAgeSeconds {
    ...
    private func scheduleDNSStartupDrainIdleClose(client: NWConnection, state: UDPStreamState) {
        ...
        let work = DispatchWorkItem { [weak self, weak state] in
            ...
            if let startedAt = state.dnsStartupDrainStartedAt,
               Date().timeIntervalSince(startedAt) >= NimaConstants.dnsStartupDrainMaxAgeSeconds {
        ...
        queue.asyncAfter(deadline: .now() + NimaConstants.dnsStartupDrainIdleSeconds, execute: work)
    }
    ...
    private func maybeTriggerUDPStartupGuardEscapeHatch(reason: String, now: Date = Date()) -> Bool {
        ...
        guard pendingUDPControlQueue.count >= NimaConstants.safeModeMaxQueuedUDPControlStreams else { return false }
    ...
    private func dnsDedupWindowForStream(streamID: Int) -> TimeInterval {
        guard let state = udpStreamsByID[streamID], state.hardeningEnabled else {
            return NimaConstants.dnsDedupWindow
        }
        return NimaConstants.tiktokHardeningDNSDedupWindow
    }
        ...
        for state in udpStreamsByID.values where !state.closed {
            ...
            if let started = state.processingStartedAt,
               now.timeIntervalSince(started) > watchdogTimeoutForState() {
                if now.timeIntervalSince(lastStuckProcessingReclaimAt) < NimaConstants.udpEmergencyReclaimMinInterval {
                    continue
                }
            ...
            if isStormMode() && (state.trafficClass == .unknown || state.trafficClass == .generic) {
                maxLifetime = NimaConstants.udpStormLowConfidenceMaxLifetime
            } else {
                maxLifetime = state.hardeningEnabled ? NimaConstants.tiktokHardeningMaxLifetime : NimaConstants.udpGlobalMaxLifetime
            }
            ...
            if isStormMode() && (state.trafficClass == .unknown || state.trafficClass == .generic) {
                idleTimeout = NimaConstants.udpStormLowConfidenceIdleTimeout
            } else {
                idleTimeout = state.hardeningEnabled ? NimaConstants.tiktokHardeningIdleTimeout : NimaConstants.udpGlobalIdleTimeout
            }
    ...
    private func closeStaleQueuedUDPControlStreams() {
        ...
        let maxQueueAge = isStormMode() ? NimaConstants.udpStormQueueMaxAge : NimaConstants.udpQueuedStreamMaxAge
    ...
    private func sweepInflightDNSExpirations() {
        let now = Date()
        for (key, inflight) in inflightDNSRequests where now.timeIntervalSince(inflight.startedAt) > NimaConstants.dnsInflightMaxAge {
    ...
    private func updateTransportDegradedState() {
        ...
        let isRecoveringDepth = pendingUDPControlQueue.count <= NimaConstants.degradedRecoverQueueDepth
        let timeoutStorm = timeoutRate >= effectiveDegradedTimeoutRateEnter()
        let timeoutRecovered = timeoutRate <= NimaConstants.degradedTimeoutRateRecover
        let badLenStorm = badLenRate >= NimaConstants.degradedBadLenRateEnter
        let badLenRecovered = badLenRate <= NimaConstants.degradedBadLenRateRecover
        let recentEmergencyReclaims = countRecentEmergencyReclaims(windowSeconds: NimaConstants.trippedWindowSeconds)
        ...
        let severeBadLenStorm = badLenRate >= NimaConstants.trippedBadLenRateEnter
        let severeReclaims = recentEmergencyReclaims >= NimaConstants.trippedEnterEmergencyReclaims
        ...
        let trippedRecovered = timeoutRate <= NimaConstants.trippedRecoverTimeoutRate &&
            pendingUDPControlQueue.count <= NimaConstants.trippedRecoverQueueDepth &&
            badLenRate <= NimaConstants.trippedBadLenRateRecover
        ...
        case .degraded:
            ...
            if severeCondition && severeSustained && degradedDwellMet {
                ...
                if let stableSince = degradedStableSince,
                   now.timeIntervalSince(stableSince) >= NimaConstants.degradedRecoverStabilizationSeconds {
        ...
        case .recovering:
            if isRecoveringDepth && timeoutRecovered && badLenRecovered &&
                now.timeIntervalSince(recoveringEnteredAt) >= NimaConstants.degradedRecoverStabilizationSeconds {
        ...
        case .tripped:
            ...
            if trippedRecovered && stabilizedSeconds >= NimaConstants.trippedRecoverStabilizationSeconds {
                degradedState = .recovering
                trippedSecondsTotal += stabilizedSeconds
                recoveringEnteredAt = now
                if stabilityFirstModeEnabled {
                    recoveryGraceUntil = now.addingTimeInterval(NimaConstants.stabilityFirstRecoveryGraceSeconds)
                }
            }
        }
    ...
    private func emitPressureDiagnosticsIfNeeded(now: Date, timeoutRate: Double, badLenRate: Double) {
        ...
        guard now.timeIntervalSince(lastPressureDiagnosticsAt) >= NimaConstants.transportPressureDiagnosticsInterval else { return }
    ...
    private func updateProtectedTransportDegradedState() {
        ...
        guard hasProtectedPressureSignal else {
            defaults?.set(false, forKey: NimaConstants.vpnLifecycleTransportDegradedKey)
            defaults?.set("", forKey: NimaConstants.vpnLifecycleTransportDegradedReasonKey)
            defaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleTransportDegradedTSKey)
            return
        }
        ...
        defaults?.set(degradedReason != nil, forKey: NimaConstants.vpnLifecycleTransportDegradedKey)
        defaults?.set(degradedReason ?? "", forKey: NimaConstants.vpnLifecycleTransportDegradedReasonKey)
        defaults?.set(Date().timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleTransportDegradedTSKey)
    }
    ...
    private func applyExtensionPressureLevelOnQueue(_ level: ExtensionPressureLevel, now: Date = Date()) {
        ...
        case .critical:
            extensionPressureDiagnosticsMutedUntil = now.addingTimeInterval(NimaConstants.extensionPressureCriticalSnapshotMuteSeconds)
            dropQueuedUDPControlStreamsUnderPressure(limit: pendingUDPControlQueue.count, includeTargetClasses: true)
            reclaimUDPStreamsUnderPressure(reason: "extension_pressure")
            reclaimLowValueUDPStreamsUnderPressure(targetActive: NimaConstants.extensionPressureCriticalTargetActiveUDP, now: now)
            trimRetainedDiagnostics(now: now, aggressive: true)
        }

        UserDefaults(suiteName: NimaConstants.appGroupID)?
            .set(extensionPressureReclaimBlockedCount, forKey: NimaConstants.vpnLifecycleExtensionPressureReclaimBlockedCountKey)
        updateTransportDegradedState()
    }
    ...
    static func extensionPressureLevel(memoryMB: Double?, activeUDP: Int, queuedUDP: Int, degradedState: String) -> ExtensionPressureLevel {
        if let memoryMB, memoryMB >= NimaConstants.extensionPressureCriticalMemoryMB {
            return .critical
        }
        if queuedUDP >= NimaConstants.extensionPressureCriticalQueuedUDP || activeUDP >= NimaConstants.extensionPressureCriticalActiveUDP || degradedState == "tripped" {
            return .critical
        }
        if let memoryMB, memoryMB >= NimaConstants.extensionPressureHardMemoryMB {
            return .hard
        }
        if queuedUDP >= NimaConstants.extensionPressureHardQueuedUDP || activeUDP >= NimaConstants.extensionPressureHardActiveUDP || degradedState == "degraded" {
            return .hard
        }
        if let memoryMB, memoryMB >= NimaConstants.extensionPressureSoftMemoryMB {
            return .soft
        }
        if queuedUDP >= max(1, NimaConstants.maxQueuedUDPControlStreams / 2) || degradedState == "recovering" {
            return .soft
        }
        return .normal
    }
    ...
    private func trimRetainedDiagnostics(now: Date = Date(), aggressive: Bool = false) {
        let hintLimit = aggressive ? max(16, NimaConstants.extensionPressureMaxClassHints / 2) : NimaConstants.extensionPressureMaxClassHints
        ...
        let suppressionLimit = aggressive ? max(16, NimaConstants.extensionPressureMaxSuppressionEntries / 2) : NimaConstants.extensionPressureMaxSuppressionEntries
        ...
        recentBlockedByHost = recentBlockedByHost.filter { now.timeIntervalSince($0.value) <= NimaConstants.aggressiveBlockSuppressionStormCooldown }
        hostCooldownUntilByKey = hostCooldownUntilByKey.filter { now < $0.value }
        recentBadLenHardFailTimestamps = recentBadLenHardFailTimestamps.filter { now.timeIntervalSince($0) <= NimaConstants.badLenRateWindowSeconds }
        recentFailOpenCandidateBlockEvents = recentFailOpenCandidateBlockEvents.filter { now.timeIntervalSince($0) <= NimaConstants.protectedBlockFailOpenWindowSeconds }
        emergencyReclaimTimestamps = emergencyReclaimTimestamps.filter { now.timeIntervalSince($0) <= NimaConstants.trippedWindowSeconds }
        maintenanceReclaimTimestamps = maintenanceReclaimTimestamps.filter { now.timeIntervalSince($0) <= NimaConstants.udpMaintenanceReclaimWindowSeconds }
    ...
    private func reclaimUDPStreamsUnderPressure(reason: String) {
        ...
        guard extensionPressureReclaim ? activeUDPStreams > 0 : activeUDPStreams >= NimaConstants.maxActiveUDPControlStreams else { return }
        let now = Date()
        ...
        maintenanceReclaimTimestamps = maintenanceReclaimTimestamps.filter {
            now.timeIntervalSince($0) <= NimaConstants.udpMaintenanceReclaimWindowSeconds
        }
        if !aggressivePressure, maintenanceReclaimTimestamps.count >= NimaConstants.udpMaintenanceReclaimBudgetPerWindow {
            maintenanceReclaimBudgetExhaustedCount += 1
            let extraCooldown = Double(maintenanceReclaimBudgetExhaustedCount) * NimaConstants.udpEmergencyReclaimMinInterval
            maintenanceReclaimCooldownUntil = now.addingTimeInterval(NimaConstants.udpEmergencyReclaimMinInterval + min(extraCooldown, 3.0))
            return
        }
        if !aggressivePressure {
            guard now.timeIntervalSince(lastEmergencyReclaimAt) >= NimaConstants.udpEmergencyReclaimMinInterval else { return }
        }
        lastEmergencyReclaimAt = now
        let hardQueuePressure = extensionPressureReclaim || pendingUDPControlQueue.count >= NimaConstants.maxQueuedUDPControlStreams
    ...
    private func watchdogTimeoutForState() -> TimeInterval {
        ...
        case .degraded, .tripped:
            return NimaConstants.udpProcessingWatchdogTimeoutDegraded
        case .healthy, .recovering:
            return NimaConstants.udpProcessingWatchdogTimeout
        }
    }
    ...
    static func effectiveMaxActiveUDPStreams(stormMode: Bool, safeMode: Bool) -> Int {
        if safeMode {
            return NimaConstants.safeModeMaxActiveUDPControlStreams
        }
        guard stormMode else { return NimaConstants.maxActiveUDPControlStreams }
        return max(1, NimaConstants.maxActiveUDPControlStreams - NimaConstants.udpStormReservedSlots)
    }
    ...
    private func hexPrefix(_ data: Data) -> String {
        data.prefix(NimaConstants.udpRecoveryHexDumpMaxBytes)
    ...
    private func classHint(for host: String, now: Date = Date()) -> ClassifiedFlow? {
        ...
        if now.timeIntervalSince(hint.ts) > NimaConstants.classHintTTLSeconds {
    ...
    private func recordClassHint(host: String, trafficClass: TrafficClass, confidence: Double, now: Date = Date()) {
        ...
        if recentClassHintsByHost.count > NimaConstants.extensionPressureMaxClassHints {
            let overflow = recentClassHintsByHost.count - NimaConstants.extensionPressureMaxClassHints
            for key in recentClassHintsByHost.sorted(by: { $0.value.ts < $1.value.ts }).prefix(overflow).map(\.key) {
                recentClassHintsByHost.removeValue(forKey: key)
            }
        }
    ...
    private func evaluateUDPPolicy(
        ...
        if selectiveSafeMode, port == 443, isKnownBlockedVideoDecision(decision) {
            let expiresAt = now.addingTimeInterval(NimaConstants.dnsTikTokHintMaxTTLSeconds)
            cacheKnownBadUDP(
                host: host,
                port: port,
                decision: decision,
    ...
    private func isUnknownUDPAllow(_ decision: PolicyDecision) -> Bool {
        decision.classification.bucket == .unknown ||
            decision.classification.confidence < NimaConstants.classifyConfidenceLow
    }
    ...
    private func cacheKnownBadUDP(
        ...
        let cap = now.addingTimeInterval(NimaConstants.dnsTikTokHintMaxTTLSeconds)
        ...
        if knownBadUDPCache.count > NimaConstants.extensionPressureMaxDNSHints {
            let overflow = knownBadUDPCache.count - NimaConstants.extensionPressureMaxDNSHints
    ...
    private func recordDNSHints(from payload: Data, now: Date = Date()) {
        ...
        for answer in summary.addressAnswers where answer.ttl > 0 {
            ...
            let ttl = min(TimeInterval(answer.ttl), NimaConstants.dnsTikTokHintMaxTTLSeconds)
        ...
        if dnsHintsByIP.count > NimaConstants.extensionPressureMaxDNSHints {
            let overflow = dnsHintsByIP.count - NimaConstants.extensionPressureMaxDNSHints
    ...
    private func recordTikTokIPHint(
        ...
        let cappedTTL = min(ttl, source == "retry_burst" ? NimaConstants.tiktokRetryBurstIPHintTTLSeconds : NimaConstants.tiktokIPHintTTLSeconds)
    ...
    private func recordTikTokIPHintFromSNI(ip: String, port: UInt16, sni: String, decision: PolicyDecision, now: Date = Date()) {
        ...
        recordTikTokIPHint(
            ...
            domain: sni,
            source: "sni",
            ttl: NimaConstants.tiktokIPHintTTLSeconds,
            confidence: 0.95,
            now: now
        )
    }
    ...
    private func pruneTikTokIPHintsToLimit() {
        guard tiktokIPHintsByKey.count > NimaConstants.maxTikTokIPHints else { return }
        let overflow = tiktokIPHintsByKey.count - NimaConstants.maxTikTokIPHints
    ...
    private func recordTikTokVideoBlockEventIfNeeded(_ decision: PolicyDecision, now: Date) {
        ...
        recentTikTokVideoBlockEvents = recentTikTokVideoBlockEvents.filter {
            now.timeIntervalSince($0) <= NimaConstants.tiktokRetryBurstBlockedHostWindow
        }
    ...
    private func hasTikTokVideoBlockBurst(now: Date) -> Bool {
        ...
        return recentTikTokVideoBlockEvents.count >= NimaConstants.tiktokRetryBurstBlockedHostThreshold
    }
    ...
    private func recordUnknownTikTokDirectIPAttemptAndMaybeHint(host: String, port: UInt16, decision: PolicyDecision, now: Date) {
        ...
        attempts = attempts.filter {
            now.timeIntervalSince($0) <= NimaConstants.tiktokRetryBurstUnknownIPAttemptWindow
        }
        attempts.append(now)
        recentUnknownTikTokDirectIPAttemptsByKey[key] = attempts

        guard attempts.count >= NimaConstants.tiktokRetryBurstUnknownIPAttemptThreshold else { return }
        recordTikTokIPHint(
            ip: host,
            port: port,
            domain: nil,
            source: "retry_burst",
            ttl: NimaConstants.tiktokRetryBurstIPHintTTLSeconds,
            confidence: 0.66,
            now: now
        )
    }
    ...
    private func markHostCooldown(host: String, port: UInt16, bucket: ContentBucket, now: Date = Date()) {
        let key = hostCooldownKey(host: host, port: port, bucket: bucket)
        hostCooldownUntilByKey[key] = now.addingTimeInterval(NimaConstants.hostCooldownDropSeconds)
    }
    ...
    private func shouldFailOpenProtectedBlock(host: String, port: UInt16, decision: PolicyDecision, transport: String, now: Date = Date()) -> Bool {
        ...
        recentFailOpenCandidateBlockEvents = recentFailOpenCandidateBlockEvents.filter {
            now.timeIntervalSince($0) <= NimaConstants.protectedBlockFailOpenWindowSeconds
        }
        recentFailOpenCandidateBlockEvents.append(now)
        guard recentFailOpenCandidateBlockEvents.count >= NimaConstants.protectedBlockFailOpenTriggerCount else {
            return false
        }
        protectedBlockFailOpenUntil = now.addingTimeInterval(NimaConstants.protectedBlockFailOpenSeconds)
        ...
        log.log(
            ...
            "reason=\(decision.reason) recent_blocks=\(recentFailOpenCandidateBlockEvents.count) duration_s=\(Int(NimaConstants.protectedBlockFailOpenSeconds))"
        )
        return true
    }
    ...
    static func isFailOpenCandidate(_ decision: PolicyDecision) -> Bool {
        ...
        return decision.classification.confidence < NimaConstants.classifyConfidenceLow
    }
    ...
    private func suppressionCooldown(for reason: String) -> TimeInterval {
        if reason == "tiktok_video_block_now" ||
            ...
            if extensionPressureLevel.rank >= ExtensionPressureLevel.hard.rank {
                return NimaConstants.aggressiveBlockSuppressionStormCooldown
            }
            switch degradedState {
            case .healthy:
    ...
    private func shouldRejectNewProtectedUDPControlStream() -> Bool {
        ...
        if phase == .critical {
            ...
            return Self.shouldForceGlobalUDPReject(
                active: activeUDPStreams,
                queued: pendingUDPControlQueue.count,
                maxActive: NimaConstants.maxActiveUDPControlStreams,
                maxQueued: NimaConstants.maxQueuedUDPControlStreams
            )
        }
        if phase == .degraded {
            return pendingUDPControlQueue.count >= NimaConstants.degradedTikTokUDPRejectQueueDepth
        }
        return false
    }
    ...
    private func shouldDropProtectedRetryByTokenBucket(host: String, port: UInt16, decision: PolicyDecision, transport: String) -> Bool {
        ...
        var bucket = tokenBucketsByHost[key] ?? TokenBucketState(
            tokens: NimaConstants.tiktokRetryTokenBucketCapacity,
            lastRefillAt: now
        )
        let elapsed = max(0, now.timeIntervalSince(bucket.lastRefillAt))
        let refill = elapsed * NimaConstants.tiktokRetryTokenBucketRefillPerSecond
        bucket.tokens = min(NimaConstants.tiktokRetryTokenBucketCapacity, bucket.tokens + refill)
    ...
    private func recentBadLenHardFailCount(now: Date = Date()) -> Int {
        recentBadLenHardFailTimestamps = recentBadLenHardFailTimestamps.filter {
            now.timeIntervalSince($0) <= NimaConstants.badLenRateWindowSeconds
        }
    ...
    private func badLenHardFailRate(now: Date = Date()) -> Double {
        ...
        let denom = max(1.0, NimaConstants.badLenRateWindowSeconds)
    ...
    private func isDNSStartupDrainWindowActive(now: Date = Date()) -> Bool {
        ...
        return now.timeIntervalSince(udpSessionStartedAt) <= NimaConstants.dnsStartupDrainWindowSeconds
    }
    ...
    private func persistDNSStartupDrainState(now: Date = Date()) {
        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        defaults?.set(isDNSStartupDrainActive(now: now), forKey: NimaConstants.vpnLifecycleDNSStartupDrainActiveKey)
        defaults?.set(dnsStartupDrainCloses, forKey: NimaConstants.vpnLifecycleDNSStartupDrainClosesKey)
        defaults?.set(dnsStartupDrainFramesProcessed, forKey: NimaConstants.vpnLifecycleDNSStartupDrainFramesProcessedKey)
    }
    ...
    private func effectiveDegradedEnterQueueDepth() -> Int {
        stabilityFirstModeEnabled ? NimaConstants.degradedEnterQueueDepth + 8 : NimaConstants.degradedEnterQueueDepth
    }

    private func effectiveTrippedEnterQueueDepth() -> Int {
        stabilityFirstModeEnabled ? NimaConstants.trippedEnterQueueDepth + 8 : NimaConstants.trippedEnterQueueDepth
    }

    private func effectiveDegradedTimeoutRateEnter() -> Double {
        stabilityFirstModeEnabled ? 0.65 : NimaConstants.degradedTimeoutRateEnter
    }

    private func effectiveTrippedEnterTimeoutRate() -> Double {
        stabilityFirstModeEnabled ? 0.80 : NimaConstants.trippedEnterTimeoutRate
    }

    private func effectiveTrippedEnterMinDegradedSeconds() -> TimeInterval {
        stabilityFirstModeEnabled ? max(NimaConstants.trippedCriticalDwellSeconds, NimaConstants.trippedEnterMinDegradedSeconds * 2.0) : NimaConstants.trippedEnterMinDegradedSeconds
    }

    private func effectiveDecodeFailureCloseThreshold() -> Int {
        stabilityFirstModeEnabled ? NimaConstants.udpDecodeFailureCloseThreshold + 2 : NimaConstants.udpDecodeFailureCloseThreshold
    }

    private func effectiveBadLenSoftFailureLimit() -> Int {
        stabilityFirstModeEnabled ? NimaConstants.udpDecodeBadLenSoftFailureLimit + 4 : NimaConstants.udpDecodeBadLenSoftFailureLimit
    }
    ...
    private func shouldCloseStreamForTimeout(state: UDPStreamState, now: Date) -> Bool {
        ...
        let lifetimeOK = now.timeIntervalSince(state.createdAt) >= NimaConstants.selectiveReclaimMinLifetimeSeconds
        let idleOK = now.timeIntervalSince(state.lastActivityAt) >= NimaConstants.selectiveReclaimMinIdleSeconds
    ...
    private func shouldAllowReclaim(reason: String, now: Date) -> Bool {
        ...
        reclaimCooldownUntilByReason[reason] = now.addingTimeInterval(NimaConstants.reclaimReasonCooldownSeconds)
        return true
    }

    private func pruneSuppressionState(now: Date) {
        let maxWindow = max(blockSuppressionCooldown, NimaConstants.aggressiveBlockSuppressionStormCooldown)
    ...
    private func reconnectBreakerCooldownRemainingSec(now: Date = Date()) -> Int {
        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        let until = defaults?.double(forKey: NimaConstants.vpnLifecycleReconnectBreakerUntilTSKey) ?? 0
    ...
    private func reconnectBreakerTrips() -> Int {
        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        return defaults?.integer(forKey: NimaConstants.vpnLifecycleReconnectBreakerTripsKey) ?? 0
    }

    private func reconnectSuppressedByBreakerCount() -> Int {
        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        return defaults?.integer(forKey: NimaConstants.vpnLifecycleReconnectSuppressedByBreakerKey) ?? 0
    }

    private func reconnectBreakerBackoffStep() -> Int {
        let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
        return defaults?.integer(forKey: NimaConstants.vpnLifecycleReconnectBreakerBackoffStepKey) ?? 0
    }

    private func updateStormModeState(now: Date, timeoutRate: Double) {
        recentUDPCreateTimestamps = recentUDPCreateTimestamps.filter {
            now.timeIntervalSince($0) <= NimaConstants.udpStormWindowSeconds
        }
        if pendingUDPControlQueue.count > NimaConstants.udpStormQueueThreshold {
        ...
        if activeUDPStreams < NimaConstants.udpStormRecoveryActiveThreshold &&
            pendingUDPControlQueue.count < NimaConstants.udpStormRecoveryQueuedThreshold &&
            timeoutRate < NimaConstants.udpStormRecoveryTimeoutRateThreshold {
    ...
    static func shouldBypassGraceForStartupGuardLowConfidenceReclaim(
        ...
        guard queueDepth >= NimaConstants.safeModeMaxQueuedUDPControlStreams else { return false }
    ...
    static func shouldTriggerUDPStartupGuardEscapeHatch(
        ...
        guard startupGuardActive else { return false }
        guard activeUDPStreams >= NimaConstants.udpStartupSerialMaxActiveStreams else { return false }
        guard queueDepth >= NimaConstants.safeModeMaxQueuedUDPControlStreams else { return false }
        guard trafficClass == .unknown || trafficClass == .generic else { return false }
        guard !preservesMessagingControl else { return false }
        guard lastPort != 53 else { return false }
    ...
    static func isValidDNSFastLanePayload(_ payload: Data) -> Bool {
        payload.count >= 12 && payload.count <= NimaConstants.maxUDPFrameSize
    }
    ...
    static func udpControlClosePlan(lastPort: UInt16?, reason: String) -> UDPControlClosePlan {
        ...
        return UDPControlClosePlan(
            ...
            cancelDelaySeconds: isDNSResponse
                ? NimaConstants.udpDNSResponseCancelWatchdogDelaySeconds
                : ((isDNSTimeoutOrMalformed || isDNSStartupDrainRetire) ? NimaConstants.udpDNSDeferredCancelDelaySeconds : 0),
    ...
    static func drainActiveLimitDuringStartupGuard(
        ...
        if startupGuardActive {
            return NimaConstants.udpStartupSerialMaxActiveStreams
        }
    ...
    static func startupGraceAdjustedUDPLimits(
        ...
        if safeMode {
            return (
                max(1, min(maxActive, NimaConstants.safeModeMaxActiveUDPControlStreams)),
                max(0, min(maxQueued, NimaConstants.safeModeMaxQueuedUDPControlStreams))
            )
        }
        ...
        return (
            min(globalMaxActive, max(maxActive, NimaConstants.startupGraceUnknownGenericMinActiveUDP)),
            min(globalMaxQueued, max(maxQueued, NimaConstants.startupGraceUnknownGenericMinQueuedUDP))
        )
    }
    ...
    static func startupGraceAdjustedUDPCreateRateCapacity(
        ...
        if safeMode {
            return max(1, min(createRateCapacity, NimaConstants.safeModeUDPAdmissionCreateRateCapacity))
        }
        guard graceActive, trafficClass == .unknown || trafficClass == .generic else {
            return createRateCapacity
        }
        return min(globalCreateRateCapacity, max(createRateCapacity, NimaConstants.startupGraceUnknownGenericMinActiveUDP))
    }
    ...
    static func pressureReclaimBatchSize(
        ...
        let maxPerSweep = stormMode ? NimaConstants.udpStormSelectiveReclaimMaxPerSweep : NimaConstants.selectiveReclaimMaxPerSweep
        let emergencyBatch = NimaConstants.udpEmergencyReclaimBatchSize + (stormMode ? 2 : 0)
        ...
        let target = pressureLevel == .critical
            ? NimaConstants.extensionPressureCriticalTargetActiveUDP
            : NimaConstants.extensionPressureHardTargetActiveUDP
    ...
    static func shouldRetireBlockedStormStream(
        ...
        guard !preservesMessagingControl else { return false }
        if blockedDecisionCount < NimaConstants.blockedStormRetireThreshold {
            return false
        }
        if let secondsSinceLastSuccess, secondsSinceLastSuccess <= NimaConstants.blockedStormRetireNoProgressSeconds {
            return false
        }
        return noProgressSeconds >= NimaConstants.blockedStormRetireNoProgressSeconds
    }
    ...
    static func shouldEnterStormMode(
        ...
        var signals = 0
        if recentUDPCreateCount > NimaConstants.udpStormOpenCountThreshold {
            signals += 1
        }
        if timeoutRate > NimaConstants.udpStormTimeoutRateThreshold {
            signals += 1
        }
        if reclaimBlockedDelta > 0 {
            signals += 1
        }
        if consecutiveQueuePressureSamples >= NimaConstants.udpStormQueueConsecutiveSamples {
            signals += 1
        }
        return signals >= 2
    }
    ...
    static func shouldExitStormMode(activeUDP: Int, queuedUDP: Int, timeoutRate: Double, stableSeconds: TimeInterval) -> Bool {
        stableSeconds >= NimaConstants.udpStormExitStabilizationSeconds &&
            activeUDP < NimaConstants.udpStormRecoveryActiveThreshold &&
            queuedUDP < NimaConstants.udpStormRecoveryQueuedThreshold &&
            timeoutRate < NimaConstants.udpStormRecoveryTimeoutRateThreshold
    }
    ...
    static func admissionTrafficClass(for classified: ClassifiedFlow) -> TrafficClass {
        ...
        if classified.confidence < NimaConstants.classifyConfidenceLow || classified.trafficClass == .unknown {
            return .unknown
        }
    ...
    func testSeedTikTokIPHint(ip: String, port: UInt16 = 443, domain: String? = "v16.tiktokcdn-us.com", source: String = "dns", ttl: TimeInterval = NimaConstants.tiktokIPHintTTLSeconds, now: Date = Date()) {
        queue.sync {
            recordTikTokIPHint(
                ip: ip,
                port: port,
    ...
    private func classConfig(for trafficClass: TrafficClass) -> AppClassConfig {
        ...
        case .generic:
            return AppClassConfig(
                maxActive: max(4, NimaConstants.maxActiveUDPControlStreams - NimaConstants.genericReservedUDPActiveSlots),
                maxQueued: max(4, NimaConstants.maxQueuedUDPControlStreams - NimaConstants.genericReservedUDPQueueSlots),
                createRatePerSecond: NimaConstants.udpAdmissionCreateRatePerSecond,
                createRateCapacity: NimaConstants.udpAdmissionCreateRateCapacity
            )
        case .unknown:
            return AppClassConfig(
                maxActive: max(2, min(4, NimaConstants.maxActiveUDPControlStreams / 4)),
                maxQueued: 4,
                createRatePerSecond: max(2.0, NimaConstants.udpAdmissionCreateRatePerSecond * 0.25),
                createRateCapacity: max(2, NimaConstants.udpAdmissionCreateRateCapacity / 4)
            )
        case .tiktok:
            return AppClassConfig(
                maxActive: max(2, NimaConstants.maxActiveUDPControlStreams / 2),
                maxQueued: max(2, NimaConstants.maxQueuedUDPControlStreams / 2),
                createRatePerSecond: NimaConstants.udpAdmissionCreateRatePerSecond * 0.5,
                createRateCapacity: max(2, NimaConstants.udpAdmissionCreateRateCapacity / 2)
            )
        case .instagram, .x:
            return AppClassConfig(
                maxActive: max(2, NimaConstants.maxActiveUDPControlStreams / 2),
                maxQueued: max(2, NimaConstants.maxQueuedUDPControlStreams / 2),
                createRatePerSecond: NimaConstants.udpAdmissionCreateRatePerSecond * 0.8,
                createRateCapacity: max(2, NimaConstants.udpAdmissionCreateRateCapacity / 2)
            )
        }
    }
    ...
    private final class UDPAdmissionController {
        ...
        private var createTokens = Double(NimaConstants.udpAdmissionCreateRateCapacity)
    ...
    private func effectiveResolverHost(originalHost: String, port: UInt16) -> String {
        ...
        if googleScore >= NimaConstants.dnsFailoverScoreThreshold, cloudflareScore < googleScore {
            return "1.1.1.1"
        }
        if cloudflareScore >= NimaConstants.dnsFailoverScoreThreshold, googleScore < cloudflareScore {
            return "8.8.8.8"
        }
    ...
    private func preferredResolverHost() -> String? {
        ...
        let now = Date()
        let selected: String
        if google.failoverScore >= NimaConstants.dnsFailoverScoreThreshold, cloudflare.failoverScore < google.failoverScore {
            selected = "1.1.1.1"
        } else if cloudflare.failoverScore >= NimaConstants.dnsFailoverScoreThreshold, google.failoverScore < cloudflare.failoverScore {
            selected = "8.8.8.8"
        } else {
        ...
        let minSwitchDelay = isReturnToPreviousHealthy
            ? max(NimaConstants.resolverSwitchCooldown, NimaConstants.dnsFailoverReturnDelaySeconds)
            : NimaConstants.resolverSwitchCooldown
    ...
    private func markResolverSuccess(host: String, port: UInt16) {
        ...
        health.failoverScore = max(0, health.failoverScore - NimaConstants.dnsFailoverScoreDecayPerSuccess)
    ...
    private func isDNSResolverStormActive() -> Bool {
        let googleScore = resolverHealth["8.8.8.8"]?.failoverScore ?? 0
        let cloudflareScore = resolverHealth["1.1.1.1"]?.failoverScore ?? 0
        return googleScore >= NimaConstants.dnsFailoverScoreThreshold &&
            cloudflareScore >= NimaConstants.dnsFailoverScoreThreshold
    }
    ...
    private func connectToTarget(
        ...
        queue.asyncAfter(deadline: .now() + NimaConstants.tcpRelayTimeout, execute: timeout)
    ...
    private func recordUploadMetadataIfNeeded(_ data: Data, tracker: RelayTracker) {
        guard tracker.sni == nil && tracker.sniProbeAttempts < NimaConstants.maxSNIProbePackets else { return }
    ...
    private func relay(from source: NWConnection, to destination: NWConnection, tracker: RelayTracker, direction: RelayDirection) {
        source.receive(minimumIncompleteLength: 1, maximumLength: NimaConstants.relayBufferSize) { [weak self] data, _, isComplete, error in
            guard let self = self else { return }
