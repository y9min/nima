...
struct TunnelProtectionState {
    ...
    private func hasManualEnabledBlocker() -> Bool {
        if let manualPolicy = loadPolicy(forKey: NimaConstants.manualFeaturePolicyKey) {
            return Self.hasEnabledBlocker(in: manualPolicy)
        }
        guard let effectivePolicy = loadPolicy(forKey: NimaConstants.featurePolicyKey) else {
            return false
        }
    ...
    private func hasActiveScheduledBlocker(now: Date, calendar: Calendar) -> Bool {
        let pauseAll = defaults?.bool(forKey: NimaConstants.timeWindowsPauseAllKey) ?? false
...
final class ReelsBlockFilter: ConnectionFilter, StreamObservationRecorder, InstagramMediaHintReporting {
    ...
    private func cacheMediaGuardSuspectHost(
        ...
        if mediaGuardSuspectHosts.count > NimaConstants.maxInstagramMediaHints {
            let overflow = mediaGuardSuspectHosts.count - NimaConstants.maxInstagramMediaHints
    ...
    func recordBlockedStream(host: String, sni: String?, port: UInt16, decision: PolicyDecision, bytesDown: Int, now: Date = Date()) {
        ...
        guard bytesDown > NimaConstants.instagramAmbiguousMediaStreamBlockThreshold else { return }
        ...
        instagramMediaHints[key] = InstagramMediaHint(
            expiresAt: now.addingTimeInterval(NimaConstants.instagramMediaHintTTLSeconds),
            addedAt: now,
            confidence: 0.70
        )
        instagramMediaHintsAdded += 1
        TunnelLogger.shared.log(
            "IG_MEDIA_HINT added host=\(sniHost) port=\(port) ttl_s=\(Int(NimaConstants.instagramMediaHintTTLSeconds)) source=large_stream"
        )
    ...
    private func pruneInstagramMediaHintsToLimit() {
        guard instagramMediaHints.count > NimaConstants.maxInstagramMediaHints else { return }
        let overflow = instagramMediaHints.count - NimaConstants.maxInstagramMediaHints
    ...
    private func logPolicyDecision(action: PolicyAction, classification: FlowClassification, reason: String, host: String) {
        ...
        if policyDecisionSuppression.count > NimaConstants.extensionPressureMaxSuppressionEntries {
            let overflow = policyDecisionSuppression.count - NimaConstants.extensionPressureMaxSuppressionEntries
    ...
    private func isLargeAmbiguousInstagramMediaStream(_ host: String, isStreamEvaluation: Bool, bytesDown: Int) -> Bool {
        ...
        return bytesDown > NimaConstants.instagramAmbiguousMediaStreamBlockThreshold
    }
    ...
    private func loadCurrentPolicy(now: Date) -> (policy: FeaturePolicyV1, source: String)? {
        let manual = loadPolicy(forKey: NimaConstants.manualFeaturePolicyKey)
        let effectiveFallback = loadPolicy(forKey: NimaConstants.featurePolicyKey)
        let policyKey = manual == nil ? NimaConstants.featurePolicyKey : NimaConstants.manualFeaturePolicyKey
        ...
        let pauseAll = sharedDefaults?.bool(forKey: NimaConstants.timeWindowsPauseAllKey) ?? false
    ...
    private func loadTimeWindows() -> [TimeWindow] {
        guard let data = sharedDefaults?.data(forKey: NimaConstants.timeWindowsKey) else { return [] }
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        return (try? decoder.decode([TimeWindow].self, from: data)) ?? []
    }
    ...
    private func loadEndedWindowIDs(now: Date) -> Set<String> {
        guard let stored = sharedDefaults?.dictionary(forKey: NimaConstants.timeWindowsEndedUntilKey) as? [String: TimeInterval] else {
            return []
        }
        return Set(stored.compactMap { windowID, endTimestamp in
            endTimestamp > now.timeIntervalSince1970 ? windowID : nil
