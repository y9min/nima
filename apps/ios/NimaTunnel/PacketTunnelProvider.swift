...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    private let pathMonitorQueue = DispatchQueue(label: "NimaTunnel.PathMonitor")
    ...
    deinit {
        ...
        if sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleRunningMarkerKey) == true {
    ...
    override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        let sessionID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleSessionIDKey) ?? "unknown"
        ...
        let settings = NEPacketTunnelNetworkSettings(tunnelRemoteAddress: NimaConstants.tunnelRemoteAddress)
        let ipv4 = NEIPv4Settings(
            addresses: [NimaConstants.tunnelLocalAddress],
            subnetMasks: [NimaConstants.tunnelSubnetMask]
        )
        ipv4.includedRoutes = [NEIPv4Route.default()]
        settings.ipv4Settings = ipv4
        let dns = NEDNSSettings(servers: NimaConstants.dnsServers)
        settings.dnsSettings = dns
        settings.mtu = NimaConstants.mtu
    ...
    private func currentTun2SocksStartupMode() -> Tun2SocksStartupMode {
        let mode = Tun2SocksStartupMode.resolve(
            rawValue: sharedDefaults?.string(forKey: NimaConstants.tun2socksStartupModeKey)
        )
        sharedDefaults?.set(mode.rawValue, forKey: NimaConstants.tun2socksStartupModeKey)
        return mode
    }

    private func markTransportReady() {
        let now = Date().timeIntervalSince1970
        sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleTransportReadyKey)
        sharedDefaults?.set(now, forKey: NimaConstants.vpnLifecycleTransportReadyTSKey)
        sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleStartupProbeCompletedKey)
        sharedDefaults?.set(now, forKey: NimaConstants.vpnLifecycleStartupProbeCompletedTSKey)
    }
    ...
    private func scheduleTun2SocksLaunchIfNeeded(proxyPort: UInt16, startupMode: Tun2SocksStartupMode) {
        ...
        let delay = NimaConstants.tun2socksPostConnectLaunchDelaySeconds
    ...
    private func tun2SocksConfig(proxyPort: UInt16) -> String {
        """
        tunnel:
          mtu: \(NimaConstants.mtu)
        socks5:
          port: \(proxyPort)
          address: \(NimaConstants.socksBindAddress)
        misc:
          task-stack-size: \(NimaConstants.tun2socksTaskStackSize)
          tcp-buffer-size: \(NimaConstants.tun2socksTCPBufferSize)
          connect-timeout: \(NimaConstants.tun2socksConnectTimeout)
          read-write-timeout: \(NimaConstants.tun2socksReadWriteTimeout)
          log-level: info
        """
    }
    ...
    override func stopTunnel(with reason: NEProviderStopReason, completionHandler: @escaping () -> Void) {
        ...
        sharedDefaults?.set(stopOrigin, forKey: NimaConstants.vpnLifecycleLastStopOriginKey)
    ...
    private func classifyStopOrigin(now: Date) -> String {
        let pendingID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecyclePendingStopIDKey) ?? ""
        let pendingSource = sharedDefaults?.string(forKey: NimaConstants.vpnLifecyclePendingStopSourceKey) ?? ""
        let pendingTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecyclePendingStopTSKey) ?? 0
        defer {
            sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecyclePendingStopIDKey)
            sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecyclePendingStopSourceKey)
            sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecyclePendingStopTSKey)
        }

        guard !pendingID.isEmpty, pendingTS > 0 else {
            sharedDefaults?.set("", forKey: NimaConstants.vpnLifecycleLastStopObservedPendingIDKey)
            log.log("STOP_ORIGIN pending_stop=missing")
            return "external_or_system"
        }

        let age = now.timeIntervalSince1970 - pendingTS
        sharedDefaults?.set(pendingID, forKey: NimaConstants.vpnLifecycleLastStopObservedPendingIDKey)
    ...
    private func handleTun2SocksExit(exitCode: Int32, config: String) {
        ...
        stopPressureSampler()
        let error = NSError(
            domain: "NimaTunnel.PacketTunnel",
            code: Int(exitCode),
    ...
    private func recordLifecycleStart() {
        ...
        sharedDefaults?.set(UUID().uuidString, forKey: NimaConstants.vpnLifecycleSessionIDKey)
        finalizeStopAttributionIfNeeded(nowTS: ts)
        persistLastCompletedStopSnapshot(nowTS: ts)
        clearStopAttributionState()
        sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleLastStartTSKey)
        sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleLastHeartbeatTSKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleUnexpectedExitKey)
        sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleRunningMarkerKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleInferredCrashKey)
        sharedDefaults?.set("unknown", forKey: NimaConstants.vpnLifecycleResolvedStopClassKey)
        sharedDefaults?.set("", forKey: NimaConstants.vpnLifecycleInferredCrashReasonKey)
        sharedDefaults?.set("{}", forKey: NimaConstants.vpnLifecycleProviderHeartbeatSnapshotJSONKey)
        sharedDefaults?.set("starting", forKey: NimaConstants.vpnLifecycleStartupStabilityPhaseKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleStartupProbeCompletedKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleStartupProbeCompletedTSKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleTransportReadyKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleTransportReadyTSKey)
        let startupMode = Tun2SocksStartupMode.resolve(
            rawValue: sharedDefaults?.string(forKey: NimaConstants.tun2socksStartupModeKey)
        )
        sharedDefaults?.set(startupMode.rawValue, forKey: NimaConstants.tun2socksStartupModeKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleDNSStartupDrainActiveKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleDNSStartupDrainClosesKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleDNSStartupDrainFramesProcessedKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey)
        sharedDefaults?.set("stability_first_startup", forKey: NimaConstants.vpnLifecycleIOSSafeModeReasonKey)
    }

    private func recordProviderPhase(_ phase: String) {
        let ts = Date().timeIntervalSince1970
        sharedDefaults?.set(phase, forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey)
        sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleProviderLastPhaseTSKey)
        appendProviderPhaseRing(phase: phase, ts: ts, source: "provider")
    }

    private func recordStartupStabilityPhase(_ phase: String) {
        sharedDefaults?.set(phase, forKey: NimaConstants.vpnLifecycleStartupStabilityPhaseKey)
    }
    ...
    private func recordStartupProbe(proxyPort: UInt16, phase: String) {
        ...
        log.breadcrumb(
            "startup_probe",
            details: "phase=\(phase) proxy_port=\(proxyPort) completed=\(sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleStartupProbeCompletedKey) ?? false)",
            minInterval: 0
        )
    }

    private func ensurePathProbeSnapshotExists() {
        let existingTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey) ?? 0
        guard existingTS <= 0 else { return }
        let now = Date().timeIntervalSince1970
        sharedDefaults?.set("probe_pending", forKey: NimaConstants.vpnLifecycleLastPathStatusKey)
        sharedDefaults?.set("none", forKey: NimaConstants.vpnLifecycleLastPathUnsatisfiedReasonKey)
        sharedDefaults?.set("none", forKey: NimaConstants.vpnLifecycleLastPathInterfacesKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleLastPathIsExpensiveKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleLastPathIsConstrainedKey)
        sharedDefaults?.set(now, forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey)
    }

    private func appendProviderPhaseRing(phase: String, ts: TimeInterval, source: String) {
        var ring = decodeJSONArray(sharedDefaults?.string(forKey: NimaConstants.providerPhaseRingJSONKey))
        ...
        writeJSONObject(ring, key: NimaConstants.providerPhaseRingJSONKey)
    }
    ...
    private func recordTun2SocksStats(lastExitCode: Int?) {
        ...
        writeJSONObject(payload, key: NimaConstants.tun2socksLastStatsJSONKey)
    }
    ...
    private func recordProviderHeartbeatSnapshot(nowTS: TimeInterval, proxyPort: UInt16? = nil) {
        ...
        let payload: [String: Any] = [
            "ts": nowTS,
            "provider_phase": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey) ?? "unknown",
            "startup_stability_phase": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStartupStabilityPhaseKey) ?? "unknown",
            "startup_probe_completed": sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleStartupProbeCompletedKey) ?? false,
            ...
            "dns_startup_drain_closes": proxySnapshot?.dnsStartupDrainCloses ?? (sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleDNSStartupDrainClosesKey) ?? 0),
            "dns_startup_drain_frames_processed": proxySnapshot?.dnsStartupDrainFramesProcessed ?? (sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleDNSStartupDrainFramesProcessedKey) ?? 0),
            ...
            "early_reconnect_suppressed": sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey) ?? false,
            "ios_safe_mode_reason": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleIOSSafeModeReasonKey) ?? "",
            "last_decoder_event": decodeJSONObject(sharedDefaults?.string(forKey: NimaConstants.udpLastDecoderEventJSONKey)),
            "last_dns_close": decodeJSONObject(sharedDefaults?.string(forKey: NimaConstants.udpLastDNSCloseJSONKey)),
            "app_lifecycle": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventKey) ?? "unknown",
            "path_state": [
                "status": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathStatusKey) ?? "unknown",
                "unsatisfied_reason": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathUnsatisfiedReasonKey) ?? "none",
                "interfaces": sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathInterfacesKey) ?? "none",
                "is_expensive": sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsExpensiveKey) ?? false,
                "is_constrained": sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsConstrainedKey) ?? false,
                "updated_at": sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey) ?? 0,
            ],
        ]
        writeJSONObject(payload, key: NimaConstants.vpnLifecycleProviderHeartbeatSnapshotJSONKey)
    }
    ...
    private func recordLifecycleStop(source: String, reason: String, reasonRaw: String, exitCode: Int?, unexpectedExit: Bool) {
        ...
        let ts = Date().timeIntervalSince1970
        sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleLastStopTSKey)
        sharedDefaults?.set(reason, forKey: NimaConstants.vpnLifecycleStopReasonKey)
        sharedDefaults?.set(source, forKey: NimaConstants.vpnLifecycleStopSourceKey)
        sharedDefaults?.set(reasonRaw, forKey: NimaConstants.vpnLifecycleStopReasonRawKey)
        sharedDefaults?.set(unexpectedExit, forKey: NimaConstants.vpnLifecycleUnexpectedExitKey)
        ...
        sharedDefaults?.set(resolvedClass, forKey: NimaConstants.vpnLifecycleResolvedStopClassKey)
        if source == "inferred_crash" || source == "tun2socks_exit" || source == "cancelTunnelWithError" {
            sharedDefaults?.set(reasonRaw, forKey: NimaConstants.vpnLifecycleInferredCrashReasonKey)
        }
        if let exitCode {
            sharedDefaults?.set(exitCode, forKey: NimaConstants.vpnLifecycleLastExitCodeKey)
        }
    ...
    private func ensureStopEventExists(nowTS: TimeInterval) {
        ...
        sharedDefaults?.set(UUID().uuidString, forKey: NimaConstants.vpnLifecycleStopEventIDKey)
        sharedDefaults?.set(nowTS, forKey: NimaConstants.vpnLifecycleStopEventStartTSKey)
    ...
    private func upsertStopSignal(candidate: String, ts: TimeInterval, osRaw: String?, osName: String?, tun2socksExitCode: Int?) {
        switch candidate {
        case "os_stop_reason":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopSignalOSStopTSKey)
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopTerminalSeenStopTunnelTSKey)
            if let osRaw { sharedDefaults?.set(osRaw, forKey: NimaConstants.vpnLifecycleStopSignalOSStopReasonRawKey) }
            if let osName { sharedDefaults?.set(osName, forKey: NimaConstants.vpnLifecycleStopSignalOSStopReasonNameKey) }
        case "app_requested_stop":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopSignalAppRequestedTSKey)
        case "tun2socks_exit":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopSignalTun2SocksExitTSKey)
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopTerminalSeenTun2SocksExitTSKey)
            if let tun2socksExitCode { sharedDefaults?.set(tun2socksExitCode, forKey: NimaConstants.vpnLifecycleStopSignalTun2SocksExitCodeKey) }
        case "provider_deinit_without_stop":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopSignalProviderDeinitTSKey)
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopTerminalSeenProviderDeinitTSKey)
        default:
            break
        }
    }
    ...
    private func finalizeStopAttributionIfNeeded(nowTS: TimeInterval) {
        ...
        sharedDefaults?.set(decision.final, forKey: NimaConstants.vpnLifecycleStopCauseFinalKey)
        sharedDefaults?.set(decision.confidence, forKey: NimaConstants.vpnLifecycleStopCauseConfidenceKey)
        sharedDefaults?.set(decision.evidence, forKey: NimaConstants.vpnLifecycleStopCauseEvidenceKey)
        sharedDefaults?.set(decision.signalOrder, forKey: NimaConstants.vpnLifecycleStopCauseSignalOrderKey)
        sharedDefaults?.set(nowTS, forKey: NimaConstants.vpnLifecycleStopCauseFinalizedTSKey)
    ...
    private func persistLastCompletedStopSnapshot(nowTS: TimeInterval) {
        ...
        let finalizedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopCauseFinalizedTSKey) ?? nowTS
    ...
    private func startPressureSampler() {
        stopPressureSampler()
        let now = Date().timeIntervalSince1970
        sharedDefaults?.set(now, forKey: NimaConstants.vpnLifecycleExtensionPressureLastSampleTSKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleExtensionPressureRuntimeSecondsKey)
        sharedDefaults?.set(SOCKSProxyServer.ExtensionPressureLevel.normal.rawValue, forKey: NimaConstants.vpnLifecycleExtensionPressureLevelKey)
        sharedDefaults?.set("passive_sampler_disabled", forKey: NimaConstants.vpnLifecycleExtensionPressureActionKey)
    ...
    private func sampleExtensionPressure(sessionStartTS: TimeInterval) {
        ...
        guard runtimeSeconds >= NimaConstants.extensionPressureSamplerStartSeconds else { return }
        ...
        let stats = Socks5Tunnel.stats
        let appLifecycle = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventKey) ?? "unknown"
        let action: String
        switch level {
        ...
        sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleExtensionPressureLastSampleTSKey)
        sharedDefaults?.set(runtimeSeconds, forKey: NimaConstants.vpnLifecycleExtensionPressureRuntimeSecondsKey)
        sharedDefaults?.set(level.rawValue, forKey: NimaConstants.vpnLifecycleExtensionPressureLevelKey)
        sharedDefaults?.set(memoryMB ?? -1, forKey: NimaConstants.vpnLifecycleExtensionPressureMemoryMBKey)
        let cpuPercent = Self.cpuUsagePercent()
        sharedDefaults?.set(cpuPercent, forKey: NimaConstants.vpnLifecycleExtensionPressureCPUPercentKey)
        sharedDefaults?.set(activeUDP, forKey: NimaConstants.vpnLifecycleExtensionPressureUDPActiveKey)
        sharedDefaults?.set(queuedUDP, forKey: NimaConstants.vpnLifecycleExtensionPressureUDPQueuedKey)
        sharedDefaults?.set(degradedState, forKey: NimaConstants.vpnLifecycleExtensionPressureDegradedStateKey)
        sharedDefaults?.set(action, forKey: NimaConstants.vpnLifecycleExtensionPressureActionKey)
        sharedDefaults?.set("\(stats.up.packets)", forKey: NimaConstants.vpnLifecycleExtensionPressureTun2SocksUpPacketsKey)
        sharedDefaults?.set("\(stats.down.packets)", forKey: NimaConstants.vpnLifecycleExtensionPressureTun2SocksDownPacketsKey)
        sharedDefaults?.set(appLifecycle, forKey: NimaConstants.vpnLifecycleExtensionPressureAppLifecycleKey)
    ...
    private func recordHeartbeatTick(source: String) {
        let now = Date().timeIntervalSince1970
        let previous = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastHeartbeatTSKey) ?? 0
        sharedDefaults?.set(now, forKey: NimaConstants.vpnLifecycleLastHeartbeatTSKey)
    ...
    private func requestAutoStopIfProtectionInactive(now: Date) -> Bool {
        ...
        sharedDefaults?.set(nowTS, forKey: NimaConstants.vpnLifecyclePendingStopTSKey)
    ...
    private func markStreakDayIfPolicyEnabled(now: Date) {
        ...
        sharedDefaults?.set(data, forKey: NimaConstants.streakDaysKey)
        lastKnownEarnedStreakDate = today
    }

    private func loadStreakDayRecords() -> [TunnelStreakDayRecord] {
        guard let data = sharedDefaults?.data(forKey: NimaConstants.streakDaysKey) else {
            return []
        }
    ...
    private func handlePathUpdate(_ path: Network.NWPath) {
        ...
        sharedDefaults?.set(status, forKey: NimaConstants.vpnLifecycleLastPathStatusKey)
        sharedDefaults?.set(reason, forKey: NimaConstants.vpnLifecycleLastPathUnsatisfiedReasonKey)
        sharedDefaults?.set(interfaces.isEmpty ? "none" : interfaces, forKey: NimaConstants.vpnLifecycleLastPathInterfacesKey)
        sharedDefaults?.set(path.isExpensive, forKey: NimaConstants.vpnLifecycleLastPathIsExpensiveKey)
        sharedDefaults?.set(path.isConstrained, forKey: NimaConstants.vpnLifecycleLastPathIsConstrainedKey)
        sharedDefaults?.set(Date().timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey)
        log.breadcrumb(
            "path_state",
    ...
    private func logPathSnapshot(prefix: String) {
        let status = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathStatusKey) ?? "unknown"
        let reason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathUnsatisfiedReasonKey) ?? "none"
        let interfaces = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathInterfacesKey) ?? "unknown"
        let expensive = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsExpensiveKey) ?? false
        let constrained = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsConstrainedKey) ?? false
        let ts = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey) ?? 0
