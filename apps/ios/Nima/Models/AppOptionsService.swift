...
final class AppOptionsService {
    ...
    private let defaults = UserDefaults(suiteName: NimaConstants.appGroupID)
    ...
    private func loadSavedStates() {
        let persistedManual = loadPersistedPolicy(forKey: NimaConstants.manualFeaturePolicyKey)
        let legacyEffective = loadPersistedPolicy(forKey: NimaConstants.featurePolicyKey)
    ...
    private func persistEffectivePolicy(updatedBy: String) -> FeaturePolicyV1 {
        ...
        let currentRevision = loadPersistedPolicy(forKey: NimaConstants.featurePolicyKey)?.revision ?? 0
        ...
        persistPolicy(effectivePolicy, forKey: NimaConstants.featurePolicyKey)
        refreshOptionStates(from: effectivePolicy)
        return effectivePolicy
    }

    private func persistManualPolicy() {
        persistPolicy(manualPolicy, forKey: NimaConstants.manualFeaturePolicyKey)
    }
...
enum AppDiagnosticsLogger {
    ...
    private static var fileURL: URL? {
        FileManager.default
            .containerURL(forSecurityApplicationGroupIdentifier: NimaConstants.appGroupID)?
            .appendingPathComponent(NimaConstants.appLogFileName)
    }
