...
final class VPNManager: ObservableObject {
    ...
    private func applyDiagnosticProfileConfiguration() {
        let enabled = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDiagnosticProfileEnabledKey) ?? false
        if enabled {
            sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleDiagnosticHoldEnabledKey)
            sharedDefaults?.set(NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds, forKey: NimaConstants.vpnLifecycleDiagnosticHoldSecondsKey)
            sharedDefaults?.set("manual_profile", forKey: NimaConstants.vpnLifecycleDiagnosticModeSourceKey)
        } else {
            let autoUntil = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDiagnosticAutoEnabledUntilTSKey) ?? 0
            if autoUntil > Date().timeIntervalSince1970 {
                sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleDiagnosticHoldEnabledKey)
                sharedDefaults?.set(NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds, forKey: NimaConstants.vpnLifecycleDiagnosticHoldSecondsKey)
                sharedDefaults?.set("auto_escalation", forKey: NimaConstants.vpnLifecycleDiagnosticModeSourceKey)
            }
        }
        appendLog(
            "Diagnostic profile effective: profile_enabled=\(enabled) hold_enabled=\(diagnosticHoldEnabled()) hold_seconds=\(String(format: "%.2f", diagnosticHoldSeconds())) mode_source=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDiagnosticModeSourceKey) ?? "default_off")"
        )
    }
    ...
    private func recordAppLifecycleEvent(_ event: String, shouldLog: Bool = true, now: Date = Date()) {
        ...
        sharedDefaults?.set(state, forKey: NimaConstants.vpnLifecycleAppStateKey)
        sharedDefaults?.set(event, forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventKey)
        sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventTSKey)
        sharedDefaults?.set(protectedDataAvailable, forKey: NimaConstants.vpnLifecycleProtectedDataAvailableKey)
        sharedDefaults?.set(thermalState, forKey: NimaConstants.vpnLifecycleAppThermalStateKey)
        sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppThermalStateTSKey)

        switch event {
        case "active":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppLastActiveTSKey)
        case "inactive":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppLastInactiveTSKey)
        case "background":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppLastBackgroundTSKey)
        case "foreground":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppLastForegroundTSKey)
        case "protected_data_available":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleProtectedDataAvailableTSKey)
        case "protected_data_unavailable":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleProtectedDataUnavailableTSKey)
        case "memory_warning":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleAppMemoryWarningTSKey)
        default:
            break
        }
    ...
    func appendLog(_ msg: String) {
        ...
        if statusLog.count > NimaConstants.maxStatusLogEntries {
            statusLog.removeFirst(statusLog.count - NimaConstants.maxStatusLogEntries)
        }
    ...
    private func loadVPNPreferences(
        ...
        NETunnelProviderManager.loadAllFromPreferences { [weak self] managers, error in
            Task { @MainActor [weak self] in
                ...
                self.sharedDefaults?.set(Date().timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleProfileLoadAllTSKey)
                ...
                if let existingManagers = managers, !existingManagers.isEmpty {
                    let matchingManager = existingManagers.first { manager in
                        ...
                        return providerBundleID == NimaConstants.tunnelBundleID
                    }

                    guard let mgr = matchingManager else {
                        self.preferencesLoadInFlight = false
                        self.appendLog("Found \(existingManagers.count) VPN profile(s), but none match \(NimaConstants.tunnelBundleID)")
    ...
    private func createVPNProfile() {
        ...
        newManager.localizedDescription = NimaConstants.vpnDescription
    ...
    private func applyPendingTunnelIntentOrPolicy(source: String) {
        ...
        let now = Date()
        if let lastPolicyReconcileAttemptAt, now.timeIntervalSince(lastPolicyReconcileAttemptAt) < NimaConstants.vpnLifecyclePolicyReconcileDebounceSeconds {
            ...
            Task { @MainActor [weak self] in
                ...
                manager.saveToPreferences { [weak self] error in
                    Task { @MainActor [weak self] in
                        ...
                        self.sharedDefaults?.set(Date().timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleProfileSaveTSKey)
                        self.sharedDefaults?.set(desiredSignature, forKey: NimaConstants.vpnLifecycleProfileConfigSignatureKey)
                        ...
                        manager.loadFromPreferences { [weak self] reloadError in
                            Task { @MainActor [weak self] in
                                ...
                                self.sharedDefaults?.set(Date().timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleProfileReloadTSKey)
    ...
    private func persistPendingStopIntent(source: String, now: Date = Date()) {
        ...
        sharedDefaults?.set(stopID, forKey: NimaConstants.vpnLifecyclePendingStopIDKey)
        sharedDefaults?.set(source, forKey: NimaConstants.vpnLifecyclePendingStopSourceKey)
        sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecyclePendingStopTSKey)
    ...
    private func scheduleAutoReconnectForUnexpectedStop() {
        ...
        var stopReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopReasonKey) ?? "unknown"
        var stopSource = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopSourceKey) ?? "unknown"
        var stopReasonRaw = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopReasonRawKey) ?? ""
        var unexpectedExit = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleUnexpectedExitKey) ?? false
        var inferredCrash = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleInferredCrashKey) ?? false
        let runningMarker = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleRunningMarkerKey) ?? false
        let lastHeartbeatTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastHeartbeatTSKey) ?? 0
        ...
        let pathStatus = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathStatusKey) ?? "unknown"
        let pathReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathUnsatisfiedReasonKey) ?? "none"
        let pathInterfaces = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastPathInterfacesKey) ?? "unknown"
        let pathExpensive = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsExpensiveKey) ?? false
        let pathConstrained = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsConstrainedKey) ?? false
        let pathTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey) ?? 0
        let sessionDurationSeconds = currentSessionDurationSeconds()
        let shortLivedSession = sessionDurationSeconds > 0 && sessionDurationSeconds < NimaConstants.reconnectBreakerShortSessionSeconds
        ...
        let effectiveFinalCause = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey) ??
            sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? ""
        let providerLastPhase = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey) ?? "unknown"
        let heartbeatSnapshotJSON = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleProviderHeartbeatSnapshotJSONKey) ?? "{}"
        let lastDecoderEventJSON = sharedDefaults?.string(forKey: NimaConstants.udpLastDecoderEventJSONKey) ?? "{}"
        let transportCause = classifyDisconnectTransportCause(
            ...
            tun2socksExitObserved: stopSource == "tun2socks_exit" ||
                (sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalTun2SocksExitTSKey) ?? 0) > 0,
        ...
        let lifecycleCategory = disconnectLifecycleCategory(
            finalCause: effectiveFinalCause,
            externalKillTier: sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExternalKillSignatureTierKey) ?? ExternalKillSignatureTier.none.rawValue
        )
        appendLog(
        ...
        if transportCause != "unknown" {
            sharedDefaults?.set(transportCause, forKey: NimaConstants.vpnLifecycleIOSSafeModeReasonKey)
        }
        ...
        let transportDegraded = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleTransportDegradedKey) ?? false
        let transportDegradedReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleTransportDegradedReasonKey) ?? ""
        let transportDegradedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleTransportDegradedTSKey) ?? 0
        ...
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey)
        ...
        if externalKillDelay > 0 {
            ...
            reconnectDelay = min(
                NimaConstants.autoReconnectFastPathMaxDelaySeconds,
                max(backoffSeconds, resilienceDelay)
            )
        }
        sharedDefaults?.set(reconnectDelay, forKey: NimaConstants.vpnLifecycleLastReconnectDelaySecondsKey)
    ...
    private func diagnosticHoldEnabled() -> Bool {
        sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDiagnosticHoldEnabledKey) ?? false
    }

    private func diagnosticHoldSeconds() -> TimeInterval {
        let configured = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDiagnosticHoldSecondsKey) ?? 0
        if configured > 0 { return configured }
        return diagnosticHoldEnabled() ? NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds : 0
    }

    private func attributionWindowSeconds() -> TimeInterval {
        max(NimaConstants.vpnLifecycleAttributionWindowSeconds, diagnosticHoldSeconds())
    }

    private func recordDropLoopAndAutoEscalateIfNeeded(now: Date) {
        let windowStartKey = NimaConstants.vpnLifecycleDropLoopWindowStartTSKey
        let countKey = NimaConstants.vpnLifecycleDropLoopCountKey
        let windowSeconds = NimaConstants.vpnLifecycleDiagnosticAutoEnableWindowSeconds
        ...
        guard nextCount >= NimaConstants.vpnLifecycleDiagnosticAutoEnableMinDrops else { return }
        let profileEnabled = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDiagnosticProfileEnabledKey) ?? false
        guard !profileEnabled else { return }
        let until = now.timeIntervalSince1970 + NimaConstants.vpnLifecycleDiagnosticAutoEnableWindowSeconds
        sharedDefaults?.set(until, forKey: NimaConstants.vpnLifecycleDiagnosticAutoEnabledUntilTSKey)
        sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleDiagnosticHoldEnabledKey)
        sharedDefaults?.set(NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds, forKey: NimaConstants.vpnLifecycleDiagnosticHoldSecondsKey)
        sharedDefaults?.set("auto_escalation", forKey: NimaConstants.vpnLifecycleDiagnosticModeSourceKey)
        appendLog(
            "Auto-escalation enabled full diagnostics: drop_count=\(nextCount) window_seconds=\(Int(windowSeconds)) hold_seconds=\(String(format: "%.2f", NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds)) active_until=\(formatUnixTS(until))"
        )
    }

    private func resilienceModeDelaySecondsIfNeeded() -> TimeInterval {
        guard diagnosticHoldEnabled() else { return 0 }
        guard diagnosticHoldSeconds() >= NimaConstants.vpnLifecycleDiagnosticHoldDefaultSeconds else { return 0 }
        let lastFinal = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey) ?? ""
        let evidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseEvidenceKey) ?? ""
    ...
    private func externalKillReconnectDelaySecondsIfNeeded() -> TimeInterval {
        let signatureTier = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExternalKillSignatureTierKey) ?? ExternalKillSignatureTier.none.rawValue
        let lastExternal = signatureTier != ExternalKillSignatureTier.none.rawValue
        let currentDropTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalStatusDropTSKey) ?? 0
        let currentFinal = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? ""
        ...
        let delay = Double.random(
            in: NimaConstants.vpnLifecycleExternalKillReconnectMinDelaySeconds...NimaConstants.vpnLifecycleExternalKillReconnectMaxDelaySeconds
        )
    ...
    private func externalKillReconnectSuppressionReason(now: Date = Date()) -> String? {
        ...
        guard signatureTier != ExternalKillSignatureTier.none.rawValue else {
            ...
            sharedDefaults?.set("inactive", forKey: NimaConstants.vpnLifecycleExternalKillReconnectSuppressionStateKey)
            return nil
        }
        let attempts = filteredExternalKillReconnectAttempts(now: now)
        if attempts.count >= NimaConstants.vpnLifecycleExternalKillReconnectMaxAttemptsPerWindow {
            let nextAllowed = (attempts.first ?? now.timeIntervalSince1970) + NimaConstants.vpnLifecycleExternalKillReconnectWindowSeconds
            sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleExternalKillReconnectCapActiveKey)
            sharedDefaults?.set(nextAllowed, forKey: NimaConstants.vpnLifecycleExternalKillReconnectNextAllowedTSKey)
            sharedDefaults?.set(attempts.count, forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptsInWindowKey)
            sharedDefaults?.set("suppressed_cooldown", forKey: NimaConstants.vpnLifecycleExternalKillReconnectSuppressionStateKey)
            return "cap_active attempts=\(attempts.count) next_allowed_at=\(formatUnixTS(nextAllowed))"
        }
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleExternalKillReconnectCapActiveKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleExternalKillReconnectNextAllowedTSKey)
        sharedDefaults?.set(attempts.count, forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptsInWindowKey)
        sharedDefaults?.set("active", forKey: NimaConstants.vpnLifecycleExternalKillReconnectSuppressionStateKey)
        return nil
    }
    ...
    private func registerExternalKillReconnectAttemptIfAllowed(now: Date = Date()) -> Bool {
        ...
        sharedDefaults?.set(attempts, forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptTSKey)
        sharedDefaults?.set(attempts.count, forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptsInWindowKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleExternalKillReconnectCapActiveKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleExternalKillReconnectNextAllowedTSKey)
        sharedDefaults?.set("active", forKey: NimaConstants.vpnLifecycleExternalKillReconnectSuppressionStateKey)
    ...
    private func filteredExternalKillReconnectAttempts(now: Date = Date()) -> [TimeInterval] {
        let raw = sharedDefaults?.array(forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptTSKey) as? [TimeInterval] ?? []
        let filtered = raw
            .filter { $0 > 0 && now.timeIntervalSince1970 - $0 <= NimaConstants.vpnLifecycleExternalKillReconnectWindowSeconds }
            .sorted()
        sharedDefaults?.set(filtered, forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptTSKey)
        return filtered
    }
    ...
    private func bypassExternalKillReconnectCapOnce(source: String) {
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleExternalKillReconnectCapActiveKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleExternalKillReconnectNextAllowedTSKey)
        sharedDefaults?.set("manual_bypass_once", forKey: NimaConstants.vpnLifecycleExternalKillReconnectSuppressionStateKey)
    ...
    private func clearReconnectBreakerForManualStart(source: String) {
        sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecycleReconnectBreakerUntilTSKey)
        sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecycleReconnectBreakerRecentDisconnectsKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleReconnectBreakerFailureScoreKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleReconnectBreakerBackoffStepKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey)
        reconnectBreakerLastSuppressed = false
    ...
    private func resetStopAttributionForNewSession(now: Date = Date()) {
        ...
        sharedDefaults?.set(sessionID, forKey: NimaConstants.vpnLifecycleSessionIDKey)
        clearStopAttributionState(preserveSession: true)
        sharedDefaults?.set(sessionID, forKey: NimaConstants.vpnLifecycleSessionIDKey)
        sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleLastStartTSKey)
    }

    private func finalizeInFlightStopEventBeforeReset(now: Date = Date()) {
        let eventID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopEventIDKey) ?? ""
        guard !eventID.isEmpty else { return }
        let hasFinal = !(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? "").isEmpty
        if !hasFinal {
            finalizeStopAttributionIfNeeded(eventID: eventID, now: now)
        }
        if !(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? "").isEmpty {
    ...
    private func clearStopAttributionState(preserveSession: Bool) {
        let keys = [
            NimaConstants.vpnLifecycleStopEventIDKey,
            NimaConstants.vpnLifecycleStopEventStartTSKey,
            NimaConstants.vpnLifecycleStopCauseFinalKey,
            NimaConstants.vpnLifecycleStopCauseConfidenceKey,
            NimaConstants.vpnLifecycleStopCauseEvidenceKey,
            NimaConstants.vpnLifecycleStopCauseSignalOrderKey,
            NimaConstants.vpnLifecycleStopCauseFinalizedTSKey,
            NimaConstants.vpnLifecycleStopSignalAppRequestedTSKey,
            NimaConstants.vpnLifecycleStopSignalOSStopTSKey,
            NimaConstants.vpnLifecycleStopSignalOSStopReasonRawKey,
            NimaConstants.vpnLifecycleStopSignalOSStopReasonNameKey,
            NimaConstants.vpnLifecycleStopSignalTun2SocksExitTSKey,
            NimaConstants.vpnLifecycleStopSignalTun2SocksExitCodeKey,
            NimaConstants.vpnLifecycleStopSignalProviderDeinitTSKey,
            NimaConstants.vpnLifecycleStopSignalStatusDropTSKey,
            NimaConstants.vpnLifecycleDiagnosticHoldElapsedMSKey,
            NimaConstants.vpnLifecycleDiagnosticHoldCompletedKey,
            NimaConstants.vpnLifecycleStopTerminalSeenStopTunnelTSKey,
            NimaConstants.vpnLifecycleStopTerminalSeenTun2SocksExitTSKey,
            NimaConstants.vpnLifecycleStopTerminalSeenProviderDeinitTSKey,
            NimaConstants.vpnLifecyclePendingStopIDKey,
            NimaConstants.vpnLifecyclePendingStopSourceKey,
            NimaConstants.vpnLifecyclePendingStopTSKey
        ]
        keys.forEach { sharedDefaults?.removeObject(forKey: $0) }
        if !preserveSession {
            sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecycleSessionIDKey)
        }
    }

    private func currentStopEventID() -> String {
        sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopEventIDKey) ?? ""
    }
    ...
    private func ensureStopEventExists(now: Date = Date()) {
        ...
        sharedDefaults?.set(eventID, forKey: NimaConstants.vpnLifecycleStopEventIDKey)
        sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleStopEventStartTSKey)
        sharedDefaults?.set("", forKey: NimaConstants.vpnLifecycleStopCauseFinalKey)
        sharedDefaults?.set("", forKey: NimaConstants.vpnLifecycleStopCauseConfidenceKey)
        sharedDefaults?.set("", forKey: NimaConstants.vpnLifecycleStopCauseEvidenceKey)
        sharedDefaults?.set("", forKey: NimaConstants.vpnLifecycleStopCauseSignalOrderKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleDiagnosticHoldElapsedMSKey)
        sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleDiagnosticHoldCompletedKey)
    }

    private func upsertStopSignal(candidate: String, ts: TimeInterval) {
        switch candidate {
        case "app_requested_stop":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopSignalAppRequestedTSKey)
        case "status_drop_without_stop_callback":
            sharedDefaults?.set(ts, forKey: NimaConstants.vpnLifecycleStopSignalStatusDropTSKey)
        default:
            break
        }
    }
    ...
    private func finalizeStopAttributionIfNeeded(eventID: String, now: Date = Date()) {
        guard (sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopEventIDKey) ?? "") == eventID else { return }
        let existingFinal = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? ""
        if !existingFinal.isEmpty { return }

        let eventStart = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopEventStartTSKey) ?? 0
        let appTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalAppRequestedTSKey) ?? 0
        let osTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalOSStopTSKey) ?? 0
        let tunTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalTun2SocksExitTSKey) ?? 0
        let deinitTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalProviderDeinitTSKey) ?? 0
        let dropTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalStatusDropTSKey) ?? 0
        let seenStopTunnelTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenStopTunnelTSKey) ?? 0
        let seenTun2SocksTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenTun2SocksExitTSKey) ?? 0
        let seenProviderDeinitTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenProviderDeinitTSKey) ?? 0
        let osRaw = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopSignalOSStopReasonRawKey) ?? ""
        let osName = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopSignalOSStopReasonNameKey) ?? ""
        let tunExit = sharedDefaults?.object(forKey: NimaConstants.vpnLifecycleStopSignalTun2SocksExitCodeKey) as? Int
        ...
        sharedDefaults?.set(Int(elapsed * 1000), forKey: NimaConstants.vpnLifecycleDiagnosticHoldElapsedMSKey)
        sharedDefaults?.set(holdCompleted, forKey: NimaConstants.vpnLifecycleDiagnosticHoldCompletedKey)
        if !holdCompleted {
            return
        }
        ...
        sharedDefaults?.set(final, forKey: NimaConstants.vpnLifecycleStopCauseFinalKey)
        sharedDefaults?.set(confidence, forKey: NimaConstants.vpnLifecycleStopCauseConfidenceKey)
        sharedDefaults?.set(evidence.joined(separator: ";"), forKey: NimaConstants.vpnLifecycleStopCauseEvidenceKey)
        sharedDefaults?.set(signalOrder, forKey: NimaConstants.vpnLifecycleStopCauseSignalOrderKey)
        sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleStopCauseFinalizedTSKey)
    ...
    private func persistLastCompletedStopSnapshot(now: Date = Date()) {
        let eventID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopEventIDKey) ?? ""
        let final = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? ""
        let signalOrder = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseSignalOrderKey) ?? ""
        guard !eventID.isEmpty, !final.isEmpty, !signalOrder.isEmpty else { return }
        let confidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseConfidenceKey) ?? ""
        let evidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseEvidenceKey) ?? ""
        let finalizedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopCauseFinalizedTSKey) ?? now.timeIntervalSince1970
        let previousCompletedEventID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopEventIDKey) ?? ""
        let currentSeq = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleLastCompletedStopSeqKey) ?? 0
        sharedDefaults?.set(eventID, forKey: NimaConstants.vpnLifecycleLastCompletedStopEventIDKey)
        sharedDefaults?.set(final, forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey)
        sharedDefaults?.set(confidence, forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseConfidenceKey)
        sharedDefaults?.set(evidence, forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseEvidenceKey)
        sharedDefaults?.set(signalOrder, forKey: NimaConstants.vpnLifecycleLastCompletedStopSignalOrderKey)
        sharedDefaults?.set(finalizedTS, forKey: NimaConstants.vpnLifecycleLastCompletedStopFinalizedTSKey)
        if previousCompletedEventID != eventID {
            sharedDefaults?.set(currentSeq + 1, forKey: NimaConstants.vpnLifecycleLastCompletedStopSeqKey)
        }
        let remediationPath = remediationPath(for: final)
        let nextAction = nextActionForRemediationPath(remediationPath)
        sharedDefaults?.set(remediationPath, forKey: NimaConstants.vpnLifecycleRemediationPathKey)
        sharedDefaults?.set(nextAction, forKey: NimaConstants.vpnLifecycleNextActionKey)
        sharedDefaults?.set(isDiagnosticComplete(), forKey: NimaConstants.vpnLifecycleDiagnosticCompletenessKey)
    ...
    private func nearestProfileOpDeltaMS(nowTS: TimeInterval, eventStartTS: TimeInterval) -> Int? {
        ...
        let ops = [
            sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleProfileLoadAllTSKey) ?? 0,
            sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleProfileLoadTSKey) ?? 0,
            sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleProfileSaveTSKey) ?? 0,
            sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleProfileReloadTSKey) ?? 0
        ].filter { $0 > 0 }
    ...
    private func pressureCriticalDurationSeconds(now: Date = Date()) -> Double {
        let level = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExtensionPressureLevelKey) ?? "unknown"
        guard level == "critical" else { return 0 }
        let runtime = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExtensionPressureRuntimeSecondsKey) ?? 0
        return max(0, runtime - NimaConstants.extensionPressureSamplerStartSeconds)
    }

    private func reclaimBlockedCountLastWindow(now: Date = Date()) -> Int {
        let ts = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExtensionPressureLastSampleTSKey) ?? 0
        guard ts > 0, now.timeIntervalSince1970 - ts <= NimaConstants.vpnLifecycleExternalKillSupportSignalWindowSeconds else {
            return 0
        }
        return sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleExtensionPressureReclaimBlockedCountKey) ?? 0
    }
    ...
    private func recordStatusDropTimestamp(now: Date) {
        ...
        history = history
            .filter { nowTS - $0 <= NimaConstants.vpnLifecycleExternalKillDropHistoryWindowSeconds }
            .sorted()
        sharedDefaults?.set(history, forKey: NimaConstants.vpnLifecycleStatusDropHistoryTSKey)
    }

    private func statusDropHistory(now: Date = Date()) -> [TimeInterval] {
        let nowTS = now.timeIntervalSince1970
        let raw = sharedDefaults?.array(forKey: NimaConstants.vpnLifecycleStatusDropHistoryTSKey) as? [TimeInterval] ?? []
        let filtered = raw
            .filter { $0 > 0 && nowTS - $0 <= NimaConstants.vpnLifecycleExternalKillDropHistoryWindowSeconds }
            .sorted()
        sharedDefaults?.set(filtered, forKey: NimaConstants.vpnLifecycleStatusDropHistoryTSKey)
        return filtered
    }
    ...
    private func externalKillSignatureTier(finalCause: String, evidence: String, now: Date = Date()) -> ExternalKillSignatureTier {
        ...
        let heartbeatAgeSeconds = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExternalKillHeartbeatStaleSecondsAtDropKey) ?? 0
        let staleHeartbeat = heartbeatAgeSeconds >= NimaConstants.vpnLifecycleExternalKillStaleHeartbeatSeconds
        let cadence = dropCadenceSeconds(now: now)
        let cadenceSupport = cadence != nil &&
            cadence! >= NimaConstants.vpnLifecycleExternalKillCadenceMinSeconds &&
            cadence! <= NimaConstants.vpnLifecycleExternalKillCadenceMaxSeconds
        let historyCount = statusDropHistory(now: now).count
        let repeatedSupport = historyCount >= NimaConstants.vpnLifecycleExternalKillMinDropCountForSupportSignal
        let appStateAtDrop = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryAppStateKey) ?? "unknown"
        let protectedDataAtDrop = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDropBoundaryProtectedDataAvailableKey) ?? true
    ...
    private func persistExternalKillAssessment(finalCause: String, evidence: String, now: Date = Date()) {
        ...
        sharedDefaults?.set(isSignature, forKey: NimaConstants.vpnLifecycleExternalKillSignatureKey)
        sharedDefaults?.set(tier.rawValue, forKey: NimaConstants.vpnLifecycleExternalKillSignatureTierKey)
        if let cadence {
            sharedDefaults?.set(cadence, forKey: NimaConstants.vpnLifecycleExternalKillCadenceSecondsKey)
        }
        sharedDefaults?.set(statusDropHistory(now: now).count, forKey: NimaConstants.vpnLifecycleExternalKillDropCadenceWindowCountKey)
        sharedDefaults?.set(pressureCriticalDurationSeconds(now: now), forKey: NimaConstants.vpnLifecycleExternalKillPressureCriticalDurationSecondsKey)
        sharedDefaults?.set(reclaimBlockedCountLastWindow(now: now), forKey: NimaConstants.vpnLifecycleExternalKillReclaimBlockedCountLastWindowKey)
        if isSignature {
            sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleExternalKillDetectedTSKey)
    ...
    private func setManualOffRequested(_ value: Bool) {
        sharedDefaults?.set(value, forKey: NimaConstants.manualOffRequestedKey)
    }

    private func reconnectBreakerRemainingCooldownSeconds(now: Date = Date()) -> Int? {
        let until = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleReconnectBreakerUntilTSKey) ?? 0
    ...
    private func resetReconnectBreakerAfterHealthySessionIfNeeded(sessionDurationSeconds: TimeInterval, now: Date = Date()) {
        guard sessionDurationSeconds >= NimaConstants.reconnectBreakerHealthySessionResetSeconds else { return }
        sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecycleReconnectBreakerUntilTSKey)
        sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecycleReconnectBreakerRecentDisconnectsKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleReconnectBreakerFailureScoreKey)
        sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleReconnectBreakerBackoffStepKey)
    ...
    private func recordUnexpectedDisconnectAndTripBreakerIfNeeded(
        ...
        if isShortUnknownStatusDrop(finalCause: finalCause) {
            ...
            sharedDefaults?.set(decision.recentDropTimestamps, forKey: NimaConstants.vpnLifecycleReconnectBreakerRecentDisconnectsKey)
            if decision.shouldSuppress {
                tripReconnectBreaker(
                    cooldown: Double.random(
                        in: NimaConstants.reconnectBreakerMinCooldownSeconds...NimaConstants.reconnectBreakerMaxCooldownSeconds
                    ),
                    now: now,
        ...
        let scoreKey = NimaConstants.vpnLifecycleReconnectBreakerFailureScoreKey
        let stepKey = NimaConstants.vpnLifecycleReconnectBreakerBackoffStepKey
        let score = (sharedDefaults?.integer(forKey: scoreKey) ?? 0) + 1
        sharedDefaults?.set(score, forKey: scoreKey)
        guard score >= NimaConstants.reconnectBreakerFailureScoreThreshold else { return }
        ...
        let exponential = NimaConstants.reconnectBreakerBaseCooldownSeconds * pow(2.0, Double(max(0, nextStep - 1)))
        let capped = min(exponential, NimaConstants.reconnectBreakerMaxCooldownSeconds)
        let jitter = capped * NimaConstants.reconnectBreakerJitterFraction
        let cooldown = max(
            NimaConstants.reconnectBreakerMinCooldownSeconds,
    ...
    private func shortUnknownDropDecision(finalCause: String, now: Date = Date()) -> (shouldSuppress: Bool, recentDropTimestamps: [TimeInterval]) {
        let recent = sharedDefaults?.array(forKey: NimaConstants.vpnLifecycleReconnectBreakerRecentDisconnectsKey) as? [TimeInterval] ?? []
        let nowTS = now.timeIntervalSince1970
        guard finalCause == "status_drop_without_stop_callback" else {
            let filtered = recent
                .filter { $0 > 0 && nowTS - $0 <= NimaConstants.reconnectBreakerShortUnknownDropWindowSeconds }
                .sorted()
        ...
        return (filtered.count >= NimaConstants.reconnectBreakerShortUnknownDropThreshold, filtered)
    }
    ...
    private func isShortUnknownStatusDrop(finalCause: String) -> Bool {
        ...
        let statusDropTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalStatusDropTSKey) ?? 0
        guard statusDropTS > 0 else { return false }
        let terminalStopTunnelTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenStopTunnelTSKey) ?? 0
        let terminalTun2SocksTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenTun2SocksExitTSKey) ?? 0
        let signalTun2SocksTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopSignalTun2SocksExitTSKey) ?? 0
        let terminalDeinitTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenProviderDeinitTSKey) ?? 0
    ...
    private func tripReconnectBreaker(
        ...
        let boundedCooldown = min(
            NimaConstants.reconnectBreakerMaxCooldownSeconds,
            max(NimaConstants.reconnectBreakerMinCooldownSeconds, cooldown)
        )
        let until = now.timeIntervalSince1970 + boundedCooldown
        sharedDefaults?.set(until, forKey: NimaConstants.vpnLifecycleReconnectBreakerUntilTSKey)
        let trips = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleReconnectBreakerTripsKey) ?? 0
        sharedDefaults?.set(trips + 1, forKey: NimaConstants.vpnLifecycleReconnectBreakerTripsKey)
        sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey)
        sharedDefaults?.set(reason, forKey: NimaConstants.vpnLifecycleIOSSafeModeReasonKey)
    ...
    private func recoverReconnectBreakerIfNeeded(now: Date = Date()) {
        ...
        if sessionDuration >= NimaConstants.reconnectBreakerHealthySessionResetSeconds {
            sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleReconnectBreakerFailureScoreKey)
            sharedDefaults?.set(0, forKey: NimaConstants.vpnLifecycleReconnectBreakerBackoffStepKey)
            sharedDefaults?.removeObject(forKey: NimaConstants.vpnLifecycleReconnectBreakerRecentDisconnectsKey)
            sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey)
    ...
    private func currentSessionDurationSeconds(now: Date = Date()) -> TimeInterval {
        let lastStartTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastStartTSKey) ?? 0
    ...
    private func resolveStopClassification(
        ...
        if stopSource == "stopTunnel" {
            guard expectedDisconnect else {
                ...
                sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleUnexpectedExitKey)
                sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleInferredCrashKey)
                sharedDefaults?.set(inferredReason, forKey: NimaConstants.vpnLifecycleInferredCrashReasonKey)
                sharedDefaults?.set(ResolvedStopClass.inferredCrash.rawValue, forKey: NimaConstants.vpnLifecycleResolvedStopClassKey)
            ...
            sharedDefaults?.set(ResolvedStopClass.cleanStop.rawValue, forKey: NimaConstants.vpnLifecycleResolvedStopClassKey)
        ...
        if runningMarker && (stopSource == "unknown" || stopReason == "unknown") {
            let inferredReason = heartbeatAge >= NimaConstants.lifecycleHeartbeatStaleSeconds
                ? "running_marker_stale_heartbeat"
                : "running_marker_unknown_stop"
            sharedDefaults?.set("inferred_crash", forKey: NimaConstants.vpnLifecycleStopSourceKey)
            sharedDefaults?.set("inferred_crash_unknown_stop", forKey: NimaConstants.vpnLifecycleStopReasonKey)
            sharedDefaults?.set(inferredReason, forKey: NimaConstants.vpnLifecycleStopReasonRawKey)
            sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleUnexpectedExitKey)
            sharedDefaults?.set(true, forKey: NimaConstants.vpnLifecycleInferredCrashKey)
            sharedDefaults?.set(false, forKey: NimaConstants.vpnLifecycleRunningMarkerKey)
            sharedDefaults?.set(ResolvedStopClass.inferredCrash.rawValue, forKey: NimaConstants.vpnLifecycleResolvedStopClassKey)
            sharedDefaults?.set(inferredReason, forKey: NimaConstants.vpnLifecycleInferredCrashReasonKey)
        ...
        sharedDefaults?.set(ResolvedStopClass.unknown.rawValue, forKey: NimaConstants.vpnLifecycleResolvedStopClassKey)
    ...
    private func incrementReconnectSuppressedByBreaker(reason: String = "reconnect_suppressed") {
        let key = NimaConstants.vpnLifecycleReconnectSuppressedByBreakerKey
    ...
    private func shouldScheduleProbeReconnect(lastTransportSignalTS: TimeInterval) -> Bool {
        ...
        if let lastProbeReconnectAt,
           Date().timeIntervalSince(lastProbeReconnectAt) < NimaConstants.transportProbeReconnectMinIntervalSeconds {
            return false
        }
        if lastTransportSignalTS > 0 {
            let signalAge = Date().timeIntervalSince1970 - lastTransportSignalTS
            if signalAge < NimaConstants.transportProbeReconnectDelaySeconds {
                return false
            }
        }
    ...
    private func scheduleProbeReconnect() {
        reconnectTask = Task { [weak self] in
            try? await Task.sleep(nanoseconds: UInt64(NimaConstants.transportProbeReconnectDelaySeconds * 1_000_000_000))
            await MainActor.run {
                guard let self else { return }
                self.reconnectTask = nil
    ...
    private func buildDiagnosticReport(container: URL) -> String {
        let tunnelFileURL = container.appendingPathComponent(NimaConstants.logFileName)
        let statsFileURL = container.appendingPathComponent(NimaConstants.statsFileName)
    ...
    private func renderLifecycleSummary() -> String {
        let eventIDForRender = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopEventIDKey) ?? ""
        let eventStartForRender = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopEventStartTSKey) ?? 0
        let hasFinal = !(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? "").isEmpty
        ...
        let lastStart = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastStartTSKey) ?? 0
        let lastStop = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastStopTSKey) ?? 0
        let lastHeartbeat = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastHeartbeatTSKey) ?? 0
        let stopReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopReasonKey) ?? "unknown"
        let stopSource = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopSourceKey) ?? "unknown"
        let stopReasonRaw = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopReasonRawKey) ?? ""
        let stopOrigin = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastStopOriginKey) ?? "unknown"
        let observedPendingStopID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastStopObservedPendingIDKey) ?? ""
        let pendingStopID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecyclePendingStopIDKey) ?? ""
        let pendingStopSource = sharedDefaults?.string(forKey: NimaConstants.vpnLifecyclePendingStopSourceKey) ?? ""
        let pendingStopTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecyclePendingStopTSKey) ?? 0
        let sessionID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleSessionIDKey) ?? ""
        let stopEventID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopEventIDKey) ?? ""
        let stopEventStartTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopEventStartTSKey) ?? 0
        let stopCauseFinal = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseFinalKey) ?? ""
        let stopCauseConfidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseConfidenceKey) ?? ""
        let stopCauseEvidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseEvidenceKey) ?? ""
        let stopSignalOrder = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopCauseSignalOrderKey) ?? ""
        let stopCauseFinalizedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopCauseFinalizedTSKey) ?? 0
        let lastCompletedStopEventID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopEventIDKey) ?? ""
        let lastCompletedStopCauseFinal = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey) ?? ""
        let lastCompletedStopCauseConfidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseConfidenceKey) ?? ""
        let lastCompletedStopCauseEvidence = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseEvidenceKey) ?? ""
        let lastCompletedStopSignalOrder = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopSignalOrderKey) ?? ""
        let lastCompletedStopFinalizedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastCompletedStopFinalizedTSKey) ?? 0
        let lastCompletedStopSeq = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleLastCompletedStopSeqKey) ?? 0
        let diagnosticHoldEnabled = diagnosticHoldEnabled()
        let diagnosticHoldSeconds = diagnosticHoldSeconds()
        let terminalSeenStopTunnelTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenStopTunnelTSKey) ?? 0
        let terminalSeenTun2SocksTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenTun2SocksExitTSKey) ?? 0
        let terminalSeenProviderDeinitTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopTerminalSeenProviderDeinitTSKey) ?? 0
        let remediationPath = remediationPath(for: lastCompletedStopCauseFinal)
        let neStopRaw = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopSignalOSStopReasonRawKey) ?? ""
        let neStopName = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStopSignalOSStopReasonNameKey) ?? ""
        let unexpectedExit = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleUnexpectedExitKey) ?? false
        let inferredCrash = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleInferredCrashKey) ?? false
        let runningMarker = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleRunningMarkerKey) ?? false
        let resolvedStopClass = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleResolvedStopClassKey) ?? lastResolvedStopClass
        let inferredCrashReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleInferredCrashReasonKey) ?? ""
        let lastExitCode = sharedDefaults?.object(forKey: NimaConstants.vpnLifecycleLastExitCodeKey) as? Int
        ...
        let pathExpensive = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsExpensiveKey) ?? false
        let pathConstrained = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleLastPathIsConstrainedKey) ?? false
        let pathUpdate = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastPathUpdateTSKey) ?? 0
        let reconnectSuppressedByBreaker = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleReconnectSuppressedByBreakerKey) ?? 0
        let diagnosticModeSource = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDiagnosticModeSourceKey) ?? "default_off"
        let diagnosticAutoEnabledUntil = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDiagnosticAutoEnabledUntilTSKey) ?? 0
        let dropLoopCount = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleDropLoopCountKey) ?? 0
        let dropLoopWindowStart = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDropLoopWindowStartTSKey) ?? 0
        let diagnosticComplete = isDiagnosticComplete()
        let nextAction = nextActionForRemediationPath(remediationPath)
        let externalKillSignature = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleExternalKillSignatureKey) ?? false
        let externalKillSignatureTier = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExternalKillSignatureTierKey) ?? ExternalKillSignatureTier.none.rawValue
        let externalKillDetectedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExternalKillDetectedTSKey) ?? 0
        let externalKillCadence = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExternalKillCadenceSecondsKey) ?? 0
        let externalKillCadenceWindowCount = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleExternalKillDropCadenceWindowCountKey) ?? 0
        let externalKillHeartbeatStaleAtDrop = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExternalKillHeartbeatStaleSecondsAtDropKey) ?? -1
        let externalKillReclaimBlocked = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleExternalKillReclaimBlockedCountLastWindowKey) ?? 0
        let externalKillCriticalDuration = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExternalKillPressureCriticalDurationSecondsKey) ?? 0
        let externalKillReconnectAttempts = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleExternalKillReconnectAttemptsInWindowKey) ?? 0
        let externalKillReconnectCapActive = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleExternalKillReconnectCapActiveKey) ?? false
        let externalKillReconnectNextAllowed = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExternalKillReconnectNextAllowedTSKey) ?? 0
        let externalKillReconnectSuppressionState = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExternalKillReconnectSuppressionStateKey) ?? "inactive"
        let lastReconnectDelay = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastReconnectDelaySecondsKey) ?? 0
        let startupStabilityPhase = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleStartupStabilityPhaseKey) ?? "unknown"
        let startupProbeCompleted = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleStartupProbeCompletedKey) ?? false
        let startupProbeCompletedTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStartupProbeCompletedTSKey) ?? 0
        let tun2socksStartupMode = sharedDefaults?.string(forKey: NimaConstants.tun2socksStartupModeKey) ??
            NimaConstants.tun2socksStartupModeStagedAfterConnect
        let transportReady = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleTransportReadyKey) ?? false
        let transportReadyTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleTransportReadyTSKey) ?? 0
        let dnsStartupDrainActive = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDNSStartupDrainActiveKey) ?? false
        let dnsStartupDrainCloses = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleDNSStartupDrainClosesKey) ?? 0
        let dnsStartupDrainFramesProcessed = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleDNSStartupDrainFramesProcessedKey) ?? 0
        let earlyReconnectSuppressed = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey) ?? false
        let iosSafeModeReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleIOSSafeModeReasonKey) ?? ""
        let holdElapsedMS = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleDiagnosticHoldElapsedMSKey) ?? 0
        let holdCompleted = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDiagnosticHoldCompletedKey) ?? false
        let appLifecycleEvent = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventKey) ?? "unknown"
        let appLifecycleEventTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventTSKey) ?? 0
        let lastBreadcrumb = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastBreadcrumbKey) ?? "unknown"
        let lastBreadcrumbTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleLastBreadcrumbTSKey) ?? 0
        let lastBreadcrumbDetails = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastBreadcrumbDetailsKey) ?? ""
        let providerLastPhase = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleProviderLastPhaseKey) ?? "unknown"
        let providerLastPhaseTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleProviderLastPhaseTSKey) ?? 0
        let providerPhaseRingJSON = sharedDefaults?.string(forKey: NimaConstants.providerPhaseRingJSONKey) ?? "[]"
        let providerHeartbeatSnapshotJSON = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleProviderHeartbeatSnapshotJSONKey) ?? "{}"
        let providerHeartbeatFields = providerHeartbeatSnapshotFields(providerHeartbeatSnapshotJSON)
        let udpLastControlStreamJSON = sharedDefaults?.string(forKey: NimaConstants.udpLastControlStreamJSONKey) ?? "{}"
        let udpLastDNSCloseJSON = sharedDefaults?.string(forKey: NimaConstants.udpLastDNSCloseJSONKey) ?? "{}"
        let udpLastDecoderEventJSON = sharedDefaults?.string(forKey: NimaConstants.udpLastDecoderEventJSONKey) ?? "{}"
        let tun2socksLastStatsJSON = sharedDefaults?.string(forKey: NimaConstants.tun2socksLastStatsJSONKey) ?? "{}"
        let udpCrashGuardUntil = sharedDefaults?.double(forKey: NimaConstants.udpCrashGuardUntilKey) ?? 0
        let udpCrashGuardReason = sharedDefaults?.string(forKey: NimaConstants.udpCrashGuardReasonKey) ?? ""
        let udpCrashGuardHits = sharedDefaults?.integer(forKey: NimaConstants.udpCrashGuardHitsKey) ?? 0
        let appState = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleAppStateKey) ?? "unknown"
        let protectedDataAvailable = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleProtectedDataAvailableKey) ?? false
        let thermalState = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleAppThermalStateKey) ?? "unknown"
        let memoryWarningTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleAppMemoryWarningTSKey) ?? 0
        let extensionPressureSampleTS = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExtensionPressureLastSampleTSKey) ?? 0
        let extensionPressureRuntime = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExtensionPressureRuntimeSecondsKey) ?? 0
        let extensionPressureMemory = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleExtensionPressureMemoryMBKey) ?? -1
        let scheduleSnapshot = ScheduledProtectionStateStore.snapshot(defaults: sharedDefaults)
        let integrityMissing: [String] = [
            stopEventStartTS > 0 ? nil : "stop_event_start",
            (sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryLifecycleEventKey)?.isEmpty == false) ? nil : "drop_boundary_lifecycle",
            (sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryThermalStateKey)?.isEmpty == false) ? nil : "drop_boundary_thermal",
        ...
        sharedDefaults?.set(diagnosticComplete, forKey: NimaConstants.vpnLifecycleDiagnosticCompletenessKey)
        sharedDefaults?.set(remediationPath, forKey: NimaConstants.vpnLifecycleRemediationPathKey)
        sharedDefaults?.set(nextAction, forKey: NimaConstants.vpnLifecycleNextActionKey)
        ...
        return [
            ...
            "tun2socks_launch_delay_seconds=\(String(format: "%.2f", NimaConstants.tun2socksPostConnectLaunchDelaySeconds))",
            ...
            "app_last_foreground_at=\(formatUnixTS(sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleAppLastForegroundTSKey) ?? 0))",
            "app_last_background_at=\(formatUnixTS(sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleAppLastBackgroundTSKey) ?? 0))",
            "app_last_inactive_at=\(formatUnixTS(sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleAppLastInactiveTSKey) ?? 0))",
            "app_last_protected_data_unavailable_at=\(formatUnixTS(sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleProtectedDataUnavailableTSKey) ?? 0))",
            ...
            "drop_boundary_app_state=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryAppStateKey) ?? "unknown")",
            "app_lifecycle_at_drop=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryLifecycleEventKey) ?? "unknown")",
            "protected_data_available_at_drop=\(sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleDropBoundaryProtectedDataAvailableKey) ?? false)",
            "thermal_state_at_drop=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryThermalStateKey) ?? "unknown")",
            "memory_warning_age_seconds=\((sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDropBoundaryMemoryWarningAgeSecondsKey) ?? -1) >= 0 ? String(Int(sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDropBoundaryMemoryWarningAgeSecondsKey) ?? -1)) : "unknown")",
            "drop_boundary_profile_op_delta_ms=\((sharedDefaults?.object(forKey: NimaConstants.vpnLifecycleDropBoundaryProfileOpDeltaMSKey) as? Int).map(String.init) ?? "unknown")",
            "drop_boundary_lock_sleep_hint=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleDropBoundaryLockSleepHintKey) ?? "unknown")",
            "drop_boundary_ts=\(formatUnixTS(sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleDropBoundaryTSKey) ?? 0))",
            ...
            "extension_pressure_level=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExtensionPressureLevelKey) ?? "unknown")",
            "extension_memory_mb=\(extensionPressureMemory >= 0 ? String(format: "%.1f", extensionPressureMemory) : "unknown")",
            "extension_cpu_percent=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExtensionPressureCPUPercentKey) ?? "unknown")",
            "extension_udp_active=\(sharedDefaults?.object(forKey: NimaConstants.vpnLifecycleExtensionPressureUDPActiveKey).map { "\($0)" } ?? "unknown")",
            "extension_udp_queued=\(sharedDefaults?.object(forKey: NimaConstants.vpnLifecycleExtensionPressureUDPQueuedKey).map { "\($0)" } ?? "unknown")",
            "extension_degraded_state=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExtensionPressureDegradedStateKey) ?? "unknown")",
            "extension_pressure_action=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExtensionPressureActionKey) ?? "unknown")",
            "extension_pressure_app_lifecycle=\(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExtensionPressureAppLifecycleKey) ?? "unknown")",
    ...
    private func classifyDisconnectTransportCause(
        ...
        if queueDepth >= NimaConstants.safeModeMaxQueuedUDPControlStreams &&
    ...
    private func renderRunbookSummary() -> String {
        ...
        let lastCompletedStopCauseFinal = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey) ?? ""
        let lastCompletedStopSignalOrder = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopSignalOrderKey) ?? ""
        let remediationPath = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleRemediationPathKey) ?? remediationPath(for: lastCompletedStopCauseFinal)
        let nextAction = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleNextActionKey) ?? nextActionForRemediationPath(remediationPath)
        let externalKillSignature = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleExternalKillSignatureKey) ?? false
        let externalKillTier = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleExternalKillSignatureTierKey) ?? ExternalKillSignatureTier.none.rawValue
        let iosSafeModeReason = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleIOSSafeModeReasonKey) ?? ""
        let earlyReconnectSuppressed = sharedDefaults?.bool(forKey: NimaConstants.vpnLifecycleEarlyReconnectSuppressedKey) ?? false
        return [
    ...
    private func isDiagnosticComplete() -> Bool {
        let eventID = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopEventIDKey) ?? ""
        let final = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopCauseFinalKey) ?? ""
        let order = sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleLastCompletedStopSignalOrderKey) ?? ""
        let seq = sharedDefaults?.integer(forKey: NimaConstants.vpnLifecycleLastCompletedStopSeqKey) ?? 0
    ...
    private func captureDropBoundaryContext(now: Date) {
        ...
        let eventStart = sharedDefaults?.double(forKey: NimaConstants.vpnLifecycleStopEventStartTSKey) ?? now.timeIntervalSince1970
        let profileDelta = nearestProfileOpDeltaMS(nowTS: now.timeIntervalSince1970, eventStartTS: eventStart)
        sharedDefaults?.set(state, forKey: NimaConstants.vpnLifecycleDropBoundaryAppStateKey)
        sharedDefaults?.set(lockSleepHint, forKey: NimaConstants.vpnLifecycleDropBoundaryLockSleepHintKey)
        sharedDefaults?.set(now.timeIntervalSince1970, forKey: NimaConstants.vpnLifecycleDropBoundaryTSKey)
        sharedDefaults?.set(sharedDefaults?.string(forKey: NimaConstants.vpnLifecycleAppLifecycleLastEventKey) ?? "unknown", forKey: NimaConstants.vpnLifecycleDropBoundaryLifecycleEventKey)
        sharedDefaults?.set(UIApplication.shared.isProtectedDataAvailable, forKey: NimaConstants.vpnLifecycleDropBoundaryProtectedDataAvailableKey)
        sharedDefaults?.set(currentThermalStateString(), forKey: NimaConstants.vpnLifecycleDropBoundaryThermalStateKey)
        sharedDefaults?.set(memoryWarningAge, forKey: NimaConstants.vpnLifecycleDropBoundaryMemoryWarningAgeSecondsKey)
        if heartbeatStale >= 0 {
            sharedDefaults?.set(heartbeatStale, forKey: NimaConstants.vpnLifecycleExternalKillHeartbeatStaleSecondsAtDropKey)
        }
        if let profileDelta {
            sharedDefaults?.set(profileDelta, forKey: NimaConstants.vpnLifecycleDropBoundaryProfileOpDeltaMSKey)
        }
    }

    private func renderPolicySummary() -> String {
        guard let data = sharedDefaults?.data(forKey: NimaConstants.featurePolicyKey),
              var policy = try? JSONDecoder().decode(FeaturePolicyV1.self, from: data) else {
