    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.horizontal, NimaSpacing.lg)
...
private struct StatBox: View {
    ...
    var body: some View {
        VStack(spacing: NimaSpacing.xs) {
            Text("\(value)")
                .font(NimaFonts.pupok(size: 24))
                .foregroundColor(color)
            Text(label)
                .font(NimaFonts.coolvetica(size: 12))
                .foregroundColor(NimaColors.white60)
        }
...
struct EventListView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                LazyVStack(spacing: NimaSpacing.sm) {
                        ...
                        VStack(alignment: .leading, spacing: NimaSpacing.xs) {
                            ...
                            if let bytes = event.bytesDown, bytes > 0 {
                                Text(formatBytes(bytes) + " down")
                                    .font(NimaFonts.coolvetica(size: 12))
                                    .foregroundColor(NimaColors.skyBlue)
                            }
                        }
                        .padding(NimaSpacing.sm)
                        .background(Color.white.opacity(0.1))
...
private struct TopDomainsChartView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: NimaSpacing.sm) {
            Text("Top Domains")
                .font(NimaFonts.pupok(size: 20))
                .foregroundColor(.white)

            if domains.isEmpty {
                Text("No data yet")
                    .font(NimaFonts.coolvetica(size: 14))
                    .foregroundColor(NimaColors.white60)
                    .frame(height: 150)
                    .frame(maxWidth: .infinity)
            } else {
                Chart(domains) { domain in
                    ...
                    .annotation(position: .trailing, alignment: .leading) {
                        Text("\(domain.count)x")
                            .font(NimaFonts.coolvetica(size: 10))
                            .foregroundColor(NimaColors.white60)
                    }
                }
                .chartXAxis {
            ...
            if history.isEmpty {
                ...
                Chart {
                    ForEach(Array(dataPoints.enumerated()), id: \.offset) { _, point in
                        ...
                        .foregroundStyle(NimaColors.skyBlue.opacity(0.15))
                    }
                }
                .chartYAxis {
                    AxisMarks { value in
                ...
                .chartXAxis {
                    AxisMarks { _ in
                        AxisValueLabel()
                            .foregroundStyle(NimaColors.white60)
                    }
                }
                .frame(height: 180)
            }
        ...
        VStack(alignment: .leading, spacing: NimaSpacing.sm) {
            ...
            if !recentlyClosedConns.isEmpty {
                Text("Recently Closed")
                    .font(NimaFonts.coolvetica(size: 16))
                    .foregroundColor(NimaColors.white60)
                    .padding(.top, NimaSpacing.xs)
