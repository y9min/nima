...
final class TransportProtectionDecisionTests: XCTestCase {
    ...
    func testSafeModeClampsStartupGraceUDPAdmissionLimits() {
        ...
        XCTAssertEqual(limits.maxActive, NimaConstants.safeModeMaxActiveUDPControlStreams)
        XCTAssertEqual(limits.maxQueued, NimaConstants.safeModeMaxQueuedUDPControlStreams)
        XCTAssertEqual(
            SOCKSProxyServer.startupGraceAdjustedUDPCreateRateCapacity(
                trafficClass: .unknown,
            ...
            NimaConstants.safeModeUDPAdmissionCreateRateCapacity
        )
    }
    ...
    func testStartupGuardSaturationAllowsLowConfidenceNonDNSReclaimDuringGrace() {
        XCTAssertTrue(
            SOCKSProxyServer.shouldBypassGraceForStartupGuardLowConfidenceReclaim(
                ...
                startupGuardActive: true,
                queueDepth: NimaConstants.safeModeMaxQueuedUDPControlStreams,
                trafficClass: .unknown,
                lastPort: 443,
        ...
        XCTAssertTrue(
            SOCKSProxyServer.shouldBypassGraceForStartupGuardLowConfidenceReclaim(
                ...
                startupGuardActive: true,
                queueDepth: NimaConstants.safeModeMaxQueuedUDPControlStreams + 1,
                trafficClass: .generic,
                lastPort: 3478,
                preservesMessagingControl: false
            )
    ...
    func testDNSResponseClosePlanUsesGracefulDeferredWatchdog() {
        ...
        XCTAssertEqual(plan.cancelDelaySeconds, NimaConstants.udpDNSResponseCancelWatchdogDelaySeconds)
    }
    ...
    func testDNSTimeoutMalformedClosePlanDiscardsAndDefersCancel() {
        ...
        XCTAssertEqual(timeout.cancelDelaySeconds, NimaConstants.udpDNSDeferredCancelDelaySeconds)
        XCTAssertTrue(malformed.discardTrailingFrames)
        XCTAssertEqual(malformed.cancelDelaySeconds, NimaConstants.udpDNSDeferredCancelDelaySeconds)
    }
    ...
    func testDNSStartupDrainClosePlanDiscardsAndDefersCancel() {
        ...
        XCTAssertEqual(idle.cancelDelaySeconds, NimaConstants.udpDNSDeferredCancelDelaySeconds)
    ...
    func testStartupGuardEscapeHatchSkipsProtectedTraffic() {
        ...
        XCTAssertTrue(
            SOCKSProxyServer.shouldTriggerUDPStartupGuardEscapeHatch(
                startupGuardActive: true,
                activeUDPStreams: NimaConstants.udpStartupSerialMaxActiveStreams,
                queueDepth: NimaConstants.safeModeMaxQueuedUDPControlStreams,
                trafficClass: .unknown,
                lastPort: 443,
                preservesMessagingControl: false
    ...
    func testStartupGuardDrainLimitReturnsToNormalSafeModeAfterExpiry() {
        XCTAssertEqual(
            ...
            NimaConstants.udpStartupSerialMaxActiveStreams
        )
        XCTAssertEqual(
            SOCKSProxyServer.drainActiveLimitDuringStartupGuard(
                startupGuardActive: false,
    ...
    func testProtectedFailOpenOnlyAppliesToLowConfidenceBlocks() {
        ...
        for _ in 0..<(NimaConstants.protectedBlockFailOpenTriggerCount - 1) {
            XCTAssertEqual(
    ...
    func testKnownVideoBlocksDoNotUseProtectedFailOpen() {
        ...
        for _ in 0..<NimaConstants.protectedBlockFailOpenTriggerCount {
    ...
    func testExtensionPressurePolicyLevels() {
        ...
        XCTAssertEqual(
            SOCKSProxyServer.extensionPressureLevel(memoryMB: NimaConstants.extensionPressureSoftMemoryMB, activeUDP: 1, queuedUDP: 0, degradedState: "healthy"),
            .soft
        )
        XCTAssertEqual(
            SOCKSProxyServer.extensionPressureLevel(memoryMB: NimaConstants.extensionPressureHardMemoryMB, activeUDP: 1, queuedUDP: 0, degradedState: "healthy"),
            .hard
        )
        XCTAssertEqual(
            SOCKSProxyServer.extensionPressureLevel(memoryMB: NimaConstants.extensionPressureCriticalMemoryMB, activeUDP: 1, queuedUDP: 0, degradedState: "healthy"),
            .critical
        )
        XCTAssertEqual(
            SOCKSProxyServer.extensionPressureLevel(memoryMB: nil, activeUDP: NimaConstants.maxActiveUDPControlStreams, queuedUDP: 0, degradedState: "healthy"),
            .critical
        )
    }
    ...
    func testBlockedStormRetirementRequiresPressureAndRepeatedBlockedDecisions() {
        XCTAssertTrue(
            SOCKSProxyServer.shouldRetireBlockedStormStream(
                blockedDecisionCount: NimaConstants.blockedStormRetireThreshold,
                secondsSinceLastSuccess: NimaConstants.blockedStormRetireNoProgressSeconds + 1,
                noProgressSeconds: NimaConstants.blockedStormRetireNoProgressSeconds + 1,
                degradedOrCriticalPressure: true,
                stormMode: false,
                hardeningEnabled: true,
                hardeningBucket: .tiktokVideo,
