...
final class ReelsBlockFilterPolicyTests: XCTestCase {
    ...
    func testReelsOffDisablesAmbiguousMediaBlocksAndHints() {
        ...
        let largeDecision = filter.evaluateStream(
            ...
            sni: host,
            port: 443,
            bytesDown: NimaConstants.instagramAmbiguousMediaStreamBlockThreshold + 1,
            connectionAge: 1.0,
            parallelConnections: 2,
            now: now
        )
    ...
    func testPolicyRevisionRegressionDoesNotDisableTikTokBlock() {
        ...
        defaults?.set(try? JSONEncoder().encode(newer), forKey: NimaConstants.featurePolicyKey)
        ...
        defaults?.set(try? JSONEncoder().encode(stale), forKey: NimaConstants.featurePolicyKey)
    ...
    func testTunnelProtectionStateFallsBackToEffectivePolicyWhenManualPolicyIsMissing() {
        ...
        defaults?.set(try? JSONEncoder().encode(effectivePolicy), forKey: NimaConstants.featurePolicyKey)
    ...
    private func makeProtectionDefaults(
        ...
        defaults?.removePersistentDomain(forName: suite)
        if let manualPolicy {
            defaults?.set(try? JSONEncoder().encode(manualPolicy), forKey: NimaConstants.manualFeaturePolicyKey)
        }

        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601
        defaults?.set(try? encoder.encode(windows), forKey: NimaConstants.timeWindowsKey)
        ...
        if !endedTimestamps.isEmpty {
            defaults?.set(endedTimestamps, forKey: NimaConstants.timeWindowsEndedUntilKey)
        }
        return defaults
    }
    ...
    private func persistScheduleState(
        ...
        defaults?.set(desiredVPNOn, forKey: NimaConstants.scheduleDesiredVPNOnKey)
        if let desiredUntil {
        ...
        defaults?.set(activeAppIDs, forKey: NimaConstants.scheduleActiveAppIDsKey)
        defaults?.set(activeAppIDs.isEmpty ? [] : ["tw_focus"], forKey: NimaConstants.scheduleActiveWindowIDsKey)
        if let manualOffUntil {
            defaults?.set(manualOffUntil.timeIntervalSince1970, forKey: NimaConstants.scheduleManualOffUntilTSKey)
        } else {
