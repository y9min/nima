...
final class AppSettingsStore {
    ...
    func setDisplayName(_ value: String) {
        ...
        if normalized.isEmpty {
            ...
            defaults?.set(normalized, forKey: BubbleConstants.displayNameKey)
        }
    }
    ...
    func setStreakRemindersEnabled(
        ...
        defaults?.set(enabled, forKey: BubbleConstants.streakRemindersEnabledKey)
    ...
    static func resetAdvancedDefaults(defaults: UserDefaults?) {
        ...
        defaults?.set(false, forKey: BubbleConstants.udpDisabledFastRejectEnabledKey)
        defaults?.set(
    ...
    static func windowsNotificationsEnabled(defaults: UserDefaults?) -> Bool {
        if let stored = defaults?.object(forKey: BubbleConstants.windowsNotificationsEnabledKey) as? Bool {
            return stored
        }
        return true
    }
    ...
    static func pauseIntervalMinutes(defaults: UserDefaults?) -> Int {
        let stored = defaults?.integer(forKey: BubbleConstants.pauseIntervalMinutesKey) ?? defaultPauseIntervalMinutes
    ...
    private func load() {
        displayName = defaults?.string(forKey: BubbleConstants.displayNameKey) ?? ""
        windowsNotificationsEnabled = Self.windowsNotificationsEnabled(defaults: defaults)
        if let storedStreakReminders = defaults?.object(forKey: BubbleConstants.streakRemindersEnabledKey) as? Bool {
            streakRemindersEnabled = storedStreakReminders
        } else {
            streakRemindersEnabled = Self.defaultStreakRemindersEnabled
        }

        let storedHour = defaults?.object(forKey: BubbleConstants.streakReminderHourKey) as? Int
        let storedMinute = defaults?.object(forKey: BubbleConstants.streakReminderMinuteKey) as? Int
