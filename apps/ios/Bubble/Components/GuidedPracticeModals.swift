    ...
    var body: some View {
        Group {
            if activeApps.count > 1 {
                GuidedPracticeOpenAppCoachBubble(text: "Open either app to test", scale: scale)
                    .frame(maxWidth: .infinity)
            } else {
                HStack(spacing: 64 * scale) {
                    ForEach(GuidedPracticeLaunchApp.allCases) { app in
                        Group {
                            if activeApps.contains(app) {
                                GuidedPracticeOpenAppCoachBubble(text: "Open this app to test", scale: scale)
                            } else {
                                Color.clear
                            }
                        }
...
private struct GuidedPracticeOpenAppCoachBubble: View {
    let text: String
    let scale: CGFloat
